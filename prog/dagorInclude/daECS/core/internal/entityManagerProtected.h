...
template_t instantiateTemplate(int id, bool update_queries = true);
const Template *buildTemplateByName(const char *);
template_t templateByName(const char *templ_name, EntityId eid = {}, bool update_queries = true);
...
template <typename Cb, class CbArg>
  requires eastl::is_invocable_v<Cb, const CbArg &, const QueryView &>
void performQueryES(QueryId h, Cb fun, const CbArg &, void *user_data, int min_quant = 0); // use min_quant of more than 0 for parallel
                                                                                           // for execution (each job will take at
                                                                                           // least min_quant of data)
template <typename Cb, class CbArg>
  requires eastl::is_invocable_v<Cb, const CbArg &, const QueryView &>
void performQueryEmptyAllowed(QueryId h, Cb fun, const CbArg &, void *user_data, int min_quant = 0); // use min_quant of  more than 0
...
enum class QueryApplyMode : uint8_t
{
  PartiallyResolved,
  FullyResolved
};
bool makeArchetypesQuery(archetype_t first_archetype, uint32_t index, QueryApplyMode mode);
...
RequestResources requestResources(EntityId eid, archetype_t old, template_t templId, const ComponentsInitializer &initializer,
  RequestResourcesType type, gameres_list_t *out_not_loaded_resources = nullptr);
...
void resolveQueryComponentSizes(uint32_t index);
...
dag::Vector<uint64_t> queryRequiredBitmasks;    // SoA, Bloom-filter bitmask of required components per query for fast pre-filtering
dag::Vector<component_t> firstMissingComponent; // SoA, first unresolved required component hash, 0 = none cached
...
void processAllEventsInStorage(T &storage, F &&f);
template <class T>
...
struct DelayedEntityCreation
{
  ...
  DelayedEntityCreation(EntityId eid_) : eid(eid_), currentlyCreating(false), isLoading(false) {}
  void clear();
};
