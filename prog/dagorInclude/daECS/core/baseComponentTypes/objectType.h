...
public:
  ...
  __forceinline const_iterator find_as(const HashedConstString str) const
  {
    ...
    if (DAGOR_LIKELY(!hasCollisions))
    {
      ...
      return it; // return (!str.str || strcmp(it->first.c_str(), str.str) == 0) ? it : container.end();
    }
    else
    ...
    if (DAGOR_LIKELY(!hasCollisions))
    {
      ...
      if (it != container.end() && *hashIt == str.hash)
      {
        if (it->first == str.str)
          ...
          hasCollisions = true; // we found collision!
      }
      else
      {
  ...
  Object() = default;
  Object(const Object &a) : container(a.container), hasCollisions(a.hasCollisions), hashContainer(a.hashContainer) { changeGen(); }
  Object(Object &&a) : container(eastl::move(a.container)), hashContainer(eastl::move(a.hashContainer)), hasCollisions(a.hasCollisions)
  {
    changeGen();
  }
  ...
  void setHasHashCollisions() { hasCollisions = true; }
...
{
  if (size() != a.size() || hasCollisions != a.hasCollisions)
    return false;
  if (DAGOR_LIKELY(!hasCollisions))
  {
