...
// A caller-owned entry for a run of traces on one basis (a damage model tracing one object): a
// repeat skips the classification and the inverse. The caller only owns it; the trace core is
// the writer (keyed by the basis and the resource's eps, refilled on a mismatch; a zero eps
// never matches). One thread at a time.
class CollResTraceTmCache
{
  friend class CollisionResource;
  vec4f col0, col1, col2;
  mat44f itm; // filled by the first trace of the basis that reads it (itmValid)
  float eps;
  float maxScaleSq; // the per-node reject scale after the class tightening
  bool ortho;
  bool itmValid;

public:
  CollResTraceTmCache() { memset((void *)this, 0, sizeof(*this)); } // eps 0: no basis yet
};

using CollisionNodeFilter = eastl::fixed_function<64, bool(int)>;
using CollisionNodeMask = eastl::bitvector<framemem_allocator, uintptr_t, eastl::vector<uintptr_t, framemem_allocator>>;
...
//                    type=0 (non-BLAS): per-node source face index.
//                    type=1: OPAQUE leaf token of the node's per-node BLAS chunk tree
...
//                      opaque identity.
//   bit  44      : nodeBlas flag. Historical: every live type=1 ref sets it (the combined
//                  behavior grids are gone); kept so stale refs decode as invalid.
//   bit  45      : reserved (the retired grid-selector bit).
...
//
// Go through the tri_ref:: accessors (nodeIndex / hasTri / isValid / faceIndex /
// nodeBlasToken / subTriIndex), never the bit layout, so the encoding can evolve.
...
// resource's nodeBlasBuildId at mint time, which getNodeFaceVertsByRef checks. The token fits the
// split field with room to spare.
...
// Non-BLAS make: type=0. data_offset is the per-node source face index; the
...
// BLAS chunk tree (soa4::LeafRef word). build_gen is the resource's nodeBlasBuildId
// at mint time (low NODE_BLAS_GEN_BITS).
...
// A RETIRED grid-era BLAS hit: nothing mints or decodes these any more; a stored one fails
// safe. Per-node chunk hits answer isNodeBlas().
inline constexpr bool isRetiredGridRef(tri_ref_t r) { return hasTri(r) && (r & TYPE_MASK) != 0 && (r & NODE_BLAS_MASK) == 0; }
...
// the retired grid-era token otherwise. Node-BLAS refs interleave the generation stamp in this
// field -- use nodeBlasToken(). Prefer the type-checked faceIndex().
...
// nodeBlasBuildId at decode. 0 for other refs.
...
// Rebase the nodeIndex field by `delta`, preserving lookup-token, reserved (bit 45), type,
// non-tri, and sub-tri bits. Used when re-emitting a child resource's intersection list into a parent's
...
// A rebased ref is IDENTITY-ONLY: its nodeIndex names a composite namespace while its token still addresses the CHILD's tree.
// Read tri_ref::nodeIndex from it and nothing else -- a decode would pair the foreign token with the named node, and no gate catches
// that.
