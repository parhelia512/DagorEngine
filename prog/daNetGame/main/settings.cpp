...
{
  const DataBlock *on_reload_params_blk = config_blk.getBlockByNameEx("onReloadChanges");
  if (on_reload_params_blk->isEmpty())
    return;

  debug("have changes in onReloadChanges, merge them to settings first");
  merge_data_block(config_blk, *on_reload_params_blk);
  config_blk.removeBlock("onReloadChanges");
}

static DataBlock console_config_blk_from_bin_folder()
{
  DataBlock devConfigBlk;
#if _TARGET_PC && !_TARGET_C4
  return devConfigBlk;
#endif

#if _TARGET_XBOX
  String config_blk_fn(0, "G:\\%s", get_config_filename().c_str());
#else
  String config_blk_fn = get_config_filename();
#endif
  if (dd_file_exist(config_blk_fn))
    dblk::load(devConfigBlk, config_blk_fn, dblk::ReadFlag::ROBUST);
  return devConfigBlk;
}
...
static void load_settings_and_apply_config(const char *stg_fn, const SettingsHashMap *changed_settings)
{
  ...
  dgs_apply_changes_to_config(configBlk, false);
#endif
...
void initial_load_settings()
{
  ...
  if (!settings_override_blk)
  {
    ...
    int argvIterator = 0;
    const bool hasForcedPlatformPatch = ::dgs_get_argv("config:usePlatformPatch", argvIterator);
    const bool loadConfigBlk = !hasForcedPlatformPatch;
#else
    const bool loadConfigBlk = true;
#endif
    ...
    if (loadConfigBlk)
    {
      ...
      if (dblk::load(*settings_override_blk, settings_override_filename, dblk::ReadFlag::ROBUST))
        debug("Loading (savedata) %s", settings_override_filename);
#endif
    }

    update_settings_from_reload_changes(*settings_override_blk);
  }
  ...
  load_settings_and_apply_config(stg_fn);
...
static SQInteger get_setting_by_blk_path(HSQUIRRELVM v)
{
  ...
  switch (type)
  {
    ...
    case DataBlock::TYPE_IPOINT2: Sqrat::PushVar(v, blk->getIPoint2(paramIdx)); break;
    case DataBlock::TYPE_IPOINT3: Sqrat::PushVar(v, blk->getIPoint3(paramIdx)); break;
    case DataBlock::TYPE_IPOINT4: Sqrat::PushVar(v, blk->getIPoint4(paramIdx)); break;
    case DataBlock::TYPE_POINT2: Sqrat::PushVar(v, blk->getPoint2(paramIdx)); break;
    case DataBlock::TYPE_POINT3: Sqrat::PushVar(v, blk->getPoint3(paramIdx)); break;
    case DataBlock::TYPE_POINT4: Sqrat::PushVar(v, blk->getPoint4(paramIdx)); break;
    case DataBlock::TYPE_MATRIX: Sqrat::PushVar(v, blk->getTm(paramIdx)); break;
    case DataBlock::TYPE_E3DCOLOR: Sqrat::PushVar(v, Color4(blk->getE3dcolor(paramIdx))); break;
    default:
...
static SQInteger set_setting_by_blk_path(HSQUIRRELVM v)
{
  ...
  const DataBlock *aco = ::dgs_get_settings()->getBlockByNameEx("__allowedConfigOverrides");

  for (int i = 0; i < (int)parts.size() - 1; ++i)
  {
    blk = blk->addBlock(parts[i]);
    aco = aco->getBlockByNameEx(parts[i]);
    force_apply_all = aco->getBool("__allow_force_apply_all", force_apply_all);
  }

  Sqrat::Var<Sqrat::Object> val(v, 3);
  auto valObj = val.value.GetObject();

  const char *paramName = parts.back();

  if (!force_apply_all && (!aco->getBool("__allow_all_params", false) && aco->findParam(paramName) < 0))
  ...
  switch (val.value.GetType())
  {
    ...
    case OT_STRING: blk->setStr(paramName, val.value.GetVar<const char *>().value); break;
    case OT_TABLE:
    {
...
static SQInteger save_settings_sq(HSQUIRRELVM v)
{
  bool applySettings = false;
  if (int numArgs = sq_gettop(v); numArgs > 1)
  {
    SQRAT_VERIFY(numArgs == 2);
    SQBool bArg;
    SQRAT_VERIFY(SQ_SUCCEEDED(sq_getbool(v, 2, &bArg)));
    applySettings = bArg;
  }

  save_settings(nullptr, applySettings);
  return 0;
}
...
void save_settings(const SettingsHashMap *changed_settings, bool apply_settings)
{
  ...
  if (apply_settings)
  {
    load_settings_and_apply_config(nullptr, changed_settings);
...
bool do_settings_changes_need_videomode_change(const FastNameMap &changed_fields)
{
  ...
  return changed_fields.getNameId("video/mode") >= 0
      ...
      || changed_fields.getNameId("video/dlssLegacyMode") >= 0;
  // clang-format on
}
...
SQ_DEF_AUTO_BINDING_MODULE_EX(bind_settings, "settings", sq::VM_ALL)
{
  ...
  tbl //
    ...
    .SquirrelFuncDeclString(save_settings_sq, "save_settings([apply: bool]): null")
...
#if DAGOR_DBGLEVEL > 0 && _TARGET_PC
  const bool usePlatformPatch = cmdBlk.getStr("usePlatformPatch", nullptr) != nullptr;
  const bool consolePresetSpecified = cmdBlk.getBlockByNameEx("graphics")->getStr("consolePreset", nullptr) != nullptr;

  G_ASSERTF(!usePlatformPatch || (usePlatformPatch && consolePresetSpecified),
    "-config:usePlatformPatch:t must be used in pairs with -config:graphics/consolePreset:t");

  if (usePlatformPatch && consolePresetSpecified)
  {
    DataBlock consolePresetChanges;
...
#endif
  {
    // disable auto resolution feature if resolution is explicitly specified
    const char *resolution = cmdBlk.getBlockByNameEx("video")->getStr("resolution", nullptr);
    if (resolution && strcmp(resolution, "auto") != 0)
      cmdBlk.addBlock("video")->addBool("overrideAAforAutoResolution", false);
