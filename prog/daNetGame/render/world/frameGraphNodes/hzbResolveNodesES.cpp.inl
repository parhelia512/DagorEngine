...
static void set_scope_lens_mask(const auto &mask_hndl, const dafg::multiplexing::Index &index)
{
  ...
  ShaderGlobal::set_texture(var::occlusion_lens_mask.get_var_id(), useScopeLensMask ? mask_hndl.get() : nullptr);
...
dafg::NodeHandle makeHzbResolveNode(bool beforeDynamics)
{
  ...
  if (beforeDynamics)
  {
    ...
    return ns.registerNode("hzb_resolve_node", DAFG_PP_NODE_SRC, [](dafg::Registry registry) {
      auto depthForOcclusionHndl =
        registry.create("depth_for_occlusion")
      ...
      return [depthForOcclusionHndl, depthHndl, cameraHndl, lensMaskHndl,
        ...
        ShaderGlobal::set_texture(occlusionExclusionMaskVarId, nullptr);
        ShaderGlobal::set_texture(var::occlusion_lens_mask.get_var_id(), nullptr);
      };
    });
  }
  ...
  return dafg::register_node("hzb_resolve_node", DAFG_PP_NODE_SRC, [](dafg::Registry registry) {
    ...
    auto occlusionExclusionMaskHndl = registry.create("occlusionExclusionMask")
    ...
    return [occlusionExclusionMaskHndl, cameraHndl, downsampledDepthHndl, lensMaskHndl,
...

      {
        ...
        SCOPE_RENDER_TARGET;
        d3d::set_render_target({occlusionExclusionMaskHndl.view().getTex2D(), 0, 0}, DepthAccess::RW, {});
        OcclusionExclusion evt(camera.viewTm);
        ::g_entity_mgr->broadcastEventImmediate(evt);
        ShaderGlobal::set_texture(occlusionExclusionMaskVarId, evt.rendered ? occlusionExclusionMaskHndl.get() : nullptr);
        ShaderGlobal::set_int(dynModelRenderPassVarId, 0);
      }

      occlusion->prepareNextFrame(camera.jitterPersp.zn, camera.jitterPersp.zf, downsampledDepthHndl.get(), nullptr);
...
static void create_resolve_hzb_node_es(const OnCameraNodeConstruction &evt)
{
  evt.nodes->push_back(makeHzbResolveNode(resolveHZBBeforeDynamic.get()));
}
