...
dafg::NodeHandle makeAcesFxUpdateNode()
{
  return dafg::register_node("acesfx_update_node", DAFG_PP_NODE_SRC, [](dafg::Registry registry) {
    ...
    registry.createBlob<OrderingToken>("acesfx_update_token");
    ...
    return [depthHndl, normalsHndl, cameraParamsHndl](dafg::multiplexing::Index multiplexing_index) {
      ...
      if (const auto *downsampledDepth = depthHndl.get())
        acesfx::setDepthTex(downsampledDepth);
...
static void create_aces_fx_update_node_es(const OnCameraNodeConstruction &evt) { evt.nodes->push_back(makeAcesFxUpdateNode()); }
