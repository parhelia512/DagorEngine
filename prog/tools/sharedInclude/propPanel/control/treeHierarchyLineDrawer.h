...
public:
  ...
  static void draw(const TreeNodeType &node, ImDrawList &draw_list, const ImRect &row_rect, float icon_or_text_start_x, bool open,
    ImU32 tree_line_color, ImU32 open_close_icon_inner_color)
  {
    ...
    const float centerX = floorf(row_rect.Min.x + (ImGui::GetStyle().FramePadding.x * 3.0f));
    const float centerY = floorf((row_rect.Min.y + row_rect.Max.y) / 2.0f);
    const float rectangleRadius = floorf(draw_list._Data->FontSize * 0.30f);
    const float itemSpacingY = ImGui::GetStyle().ItemSpacing.y;
    const float rowTopMinusSpacing = row_rect.Min.y - itemSpacingY;
    const float rowBottomPlusSpacing = row_rect.Max.y + itemSpacingY;
    ...
    if (nodeHasChildren(node))
    {
      ...
      {
        const float verticalLineTop = hasPreviousSibling ? row_rect.Min.y : rowTopMinusSpacing;
      ...
      if (hasNextSibling)
        draw_list.AddLine(ImVec2(centerX, centerY + rectangleRadius), ImVec2(centerX, rowBottomPlusSpacing), tree_line_color);
    }
    else
    {
      ...
      const float verticalLineTop = hasPreviousSibling ? row_rect.Min.y : rowTopMinusSpacing;
      const float verticalLineBottom = hasNextSibling ? rowBottomPlusSpacing : centerY;
    ...
    for (const TreeNodeType *currentNode = nodeGetParent(node); currentNode; currentNode = nodeGetParent(*currentNode))
    {
      ...
      {
        const float verticalLineBottom = rowBottomPlusSpacing;
        draw_list.AddLine(ImVec2(currentTreeLineCenterX, row_rect.Min.y), ImVec2(currentTreeLineCenterX, verticalLineBottom),
          tree_line_color);
      }
    }
  }
