...
CompileResult compileShaderMetalDXC(const spirv::DXCContext *dxc_ctx, const char *source, const char *profile, const char *entry,
  bool need_disasm, bool hlsl2021, bool enable_fp16, bool skipValidation, bool optimize, int implicit_cbuf_size,
  const char *shader_name, bool use_ios_token, bool use_binary_msl, uint64_t shader_variant_hash, bool enableBindless)
{
  ...
  auto hlsl2spirvResult = hlsl2spirv(dxc_ctx, source, profile, entry, hlsl2021, enable_fp16, skipValidation, implicit_cbuf_size,
    compileResult, enableBindless);
