...
void GatherVarShaderEvalCB::eval_bool_decl(bool_decl &decl)
{
  ...
  ctx.localBoolVars().add(decl.resolvedNid, decl.expr, parser, decl.name, true);

  // @TODO: do away with string autogenerated aliases -- we can embed literal name ids everywhere (and do mangling like that as well)
...
void GatherVarShaderEvalCB::decl_bool_alias(const char *name, const char *base_name)
{
  ctx.localBoolVars().addAlias(name, base_name, parser, true);
}
...
ShVarBool GatherVarShaderEvalCB::addIntervalRef(const char *intervalName, Terminal *terminal, bool_value *e)
{
  ...
  int intervalNameId = ctx.tgtCtx().intervalNameMap().getNameId(intervalName);
  ShaderVariant::ExtType intervalIndex = ctx.intervals().getIntervalIndex(intervalNameId);
  const Interval *interv = ctx.intervals().getInterval(intervalIndex);

  if (intervalIndex == INTERVAL_NOT_INIT)
  {
    intervalIndex = ctx.tgtCtx().globVars().getIntervalList().getIntervalIndex(intervalNameId);
  ...
  ctx.typeTables().uniqueIntervalNameIdsReferencedInBoolExpressions.insert(intervalNameId);
  ...
  if (auto valueMaybe = ctx.assumes().getAssumedVal(intervalNameId, is_global))
  {
...
ShVarBool GatherVarShaderEvalCB::addTextureIntervalRef(const char *textureName, Terminal *terminal, bool_value &e)
{
  ...
  auto [intervalIndex, textureNameId] = semantic::get_or_add_resource_interval(textureName, ctx.tgtCtx(), &ctx.intervals());

  if (intervalIndex == INTERVAL_NOT_INIT)
  {
    report_error(parser, terminal, "interval '%s' already exists and its type differs from the new interval!", textureName);
  ...
  if (textureNameId >= 0)
  {
    ...
    ctx.typeTables().uniqueIntervalNameIdsReferencedInBoolExpressions.insert(textureNameId);
  }
...
{
  bool wasCompilingBool = compilingBoolExpr;
  compilingBoolExpr = true;
  ...
  compilingBoolExpr = wasCompilingBool;
...
ShVarBool GatherVarShaderEvalCB::eval_bool_value(bool_value &e)
{
  ...
  {
    auto [expr, isGlobal, hasMultipleDeclarations] = semantic::get_bool_expr(*e.bool_var, parser, ctx);
    if (expr)
    {
  ...
  {
    auto [expr, isGlobal, _] = semantic::get_bool_maybe(*e.maybe_bool_var, ctx);
    if (expr)
    {
