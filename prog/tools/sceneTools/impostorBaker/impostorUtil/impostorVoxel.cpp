...
bool ImpostorBaker::bakeVoxels(dag::Span<VoxelBakeRequest> requests)
{
  ...
  // render requested views
  {
    ...
    SCOPE_RENDER_TARGET;
    get_impostor_texture_mgr()->start_rendering_slices(voxelRt.get(), /*is_voxel*/ true);
    ...
    if (0)
    {
      ...
      for (auto &r : requests)
      {
          ...
          r.colorMul->w, r.colorMul->h, r.colorMul->w * 4, nullptr, 0);
        {
          ...
          dp = image->getPixels();
          auto const *zp = r.depth->getPixels();
          for (int num = image->w * image->h; num > 0; num--, zp++, dp++)
          {
            int z = int(*zp);
            dp->r = clamp(z - 255 * 0, 0, 255);
            dp->g = clamp(z - 255 * 1, 0, 255);
            dp->b = clamp(z - 255 * 2, 0, 255);
            dp->a = *zp < 1e30f ? 255 : 0;
          }
          save_png32(String(0, "%s_%02u_d_%c.png", asset->getName(), reqCounter, "LRBTKF"[r.faceId]), image->getPixels(), image->w,
            image->h, image->w * 4, nullptr, 0);
        }
        reqCounter++;
      }
