...
{
  const char *paramsFile = CollisionPlugin::getPhysMatPath(&(DAGORED2->getConsole()));
  if (!paramsFile)
    return;

  DataBlock blk;
  DataBlock *materialsBlk, *defMatBlk;
  if (!CollisionPlugin::isValidPhysMatBlk(paramsFile, blk, NULL, &materialsBlk, &defMatBlk))
    return;
