...
bool is_box_selected_by_point_click(const BBox3 &box, const TMatrix &wtm, IGenViewportWnd *vp, int x, int y)
{
  Point3 dir, p0;
...
bool is_box_selected_by_rectangle(const BBox3 &box, const TMatrix &wtm, IGenViewportWnd *vp, const EcRect &rect)
{
  real z;
  Point2 cp[8];
  BBox2 box2;
...
void make_safe_entity_components_restored_from_undo(ECSEntityCreateData *removedData)
{
...
void proj_point(const TMatrix &wtm, const Point3 &in_from, const Point3 &in_to, Tab<Point3> &points)
{
...
template <typename T>
void iterate_scene_hierarchy_upward(ecs::EntityId eid, T &&callback)
{
  if (const ecs::Scene::EntityRecord *erecord = ecs::g_scenes->getActiveScene().findEntityRecord(eid))
  {
    ecs::Scene::SceneId sceneId = erecord->sceneId;

    while (sceneId != ecs::Scene::C_INVALID_SCENE_ID)
    {
      callback(sceneId);

      if (const ecs::Scene::SceneRecord *srecord = ecs::g_scenes->getActiveScene().getSceneRecordById(sceneId))
      {
        sceneId = srecord->parent;
      }
      else
      {
        sceneId = ecs::Scene::C_INVALID_SCENE_ID;
      }
    }
  }
}
...
void ECSEntityObject::resetObjectFlags()
{
  objectLastTemplateId = ecs::INVALID_TEMPLATE_INDEX;
  objectFlags = 0;
}

void ECSEntityObject::updateObjectFlags() const
{
...
bool ECSEntityObject::hasTransform() const { return (objectFlags & EDITABLE_OBJECT_FLAG_HAS_TRANSFORM) != 0; }

ECSEntityObject::ECSEntityObject(ecs::EntityId _id)
{
...
void ECSEntityObject::updateGeneratedName()
{
...
void ECSEntityObject::setupEid(ecs::EntityId _id)
{
  eid = _id;
...
void ECSEntityObject::updateLbb()
{
...
void ECSEntityObject::updateEntityPosition()
{
  if (hasTransform())
  {
...
void ECSEntityObject::update(float)
{
  if (hasTransform())
  {
...
bool ECSEntityObject::isSelectedByRectangle(IGenViewportWnd *vp, const EcRect &rect) const
{
...
bool ECSEntityObject::isSelectedByPointClick(IGenViewportWnd *vp, int x, int y) const
{
  if (objectFlags & EDITABLE_OBJECT_FLAG_IGNORED_EDITABLE)
    return false;
...
void ECSEntityObject::save(DataBlock &blk, const ecs::Scene::EntityRecord &erec)
{
...
void ECSEntityObject::onRemove(ObjectEditor *objEd)
{
...
void ECSEntityObject::onAdd(ObjectEditor *objEd)
{
  if (eid == ecs::INVALID_ENTITY_ID && removedEntityData) // recreate entity
  {
    ...
    ecs::Scene::SceneId prevId = ecs::g_scenes->getActiveScene().getTargetSceneId();
    if (removedEntityData->scene != ecs::Scene::C_INVALID_SCENE_ID)
    {
      ecs::g_scenes->getActiveScene().setTargetSceneId(removedEntityData->scene);
    }
    ...
    if (removedEntityData->scene != ecs::Scene::C_INVALID_SCENE_ID)
    {
      ecs::g_scenes->setEntityOrder(removedEntityData->eid, removedEntityData->orderInScene);
      ecs::g_scenes->getActiveScene().setTargetSceneId(prevId);
    }
  }
  removedEntityData.reset();
...
ECSEntityObject *ECSEntityObject::cloneObject() { return cloneObject(nullptr); }

ECSEntityObject *ECSEntityObject::cloneObject(const char *template_name)
{
  ECSEntityObject *o = nullptr;
  if (eid == ecs::INVALID_ENTITY_ID)
    o = new ECSEntityObject(ecs::INVALID_ENTITY_ID);
  else
  {
    ...
    {
      ecs::g_scenes->getActiveScene().cloneEntityRecord(eid, clone_eid, template_name);
    }

    o = new ECSEntityObject(clone_eid);
...
void ECSEntityObject::recreateObject(ObjectEditor *objEd)
{
  if (eid == ecs::INVALID_ENTITY_ID)
    return;
  if (!g_entity_mgr->doesEntityExist(eid))
...
ECSEntityObject *ECSEntityObject::getParentObject()
{
  const ecs::EntityId *parentEid = g_entity_mgr->getNullable<ecs::EntityId>(getEid(), ECS_HASH("hierarchy_parent"));
  if (!parentEid || !*parentEid)
    return nullptr;

  EditableObject *parentObject = static_cast<ECSObjectEditor *>(getObjEditor())->getObjectFromEntityId(*parentEid);
  return RTTI_cast<ECSEntityObject>(parentObject);
}

void ECSEntityObject::hideObject(bool hide)
{
  if (!isValid())
  {
    return;
  ...
  if (!hide)
  {
    if (const ecs::Scene::EntityRecord *erecord = ecs::g_scenes->getActiveScene().findEntityRecord(eid))
    {
      ...
      iterate_scene_hierarchy_upward(eid, [&ed](ecs::Scene::SceneId scene_id) {
        if (RenderableEditableObject *obj = ed.getObjectFromSceneId(scene_id))
        {
          // Use setFlags instead of hideObject of SceneObject, because hideObject will modify all children states
          obj->setFlags(0, FLG_HIDDEN);
        }
      });
    }
  }
...
void ECSEntityObject::lockObject(bool lock)
{
  if (!isValid())
  {
    return;
  ...
  if (!lock)
  {
    ...
    iterate_scene_hierarchy_upward(eid, [&ed](ecs::Scene::SceneId scene_id) {
      if (RenderableEditableObject *obj = ed.getObjectFromSceneId(scene_id))
      {
        // Use setFlags instead of lockObject of SceneObject, because lockObject will modify all children states
        obj->setFlags(0, FLG_LOCKED);
      }
    });
  }
...
void ECSEntityObject::fillProps(PropPanel::ContainerPropertyControl &panel, DClassID for_class_id,
  dag::ConstSpan<RenderableEditableObject *> objects)
{
  ECSEntityObjectPropertyPanel entityObjectPropertyPanel(*static_cast<ECSBasicObjectEditor *>(getObjEditor()), panel);

  if (objects.size() == 1 && objects[0] == this)
    entityObjectPropertyPanel.fillProps(getEid(), isLocked());
  else
    entityObjectPropertyPanel.fillProps(ecs::INVALID_ENTITY_ID);
}

void ECSEntityObject::onPPChange(int pid, bool edit_finished, PropPanel::ContainerPropertyControl &panel,
  dag::ConstSpan<RenderableEditableObject *> objects)
{
  if (objects.size() != 1 || objects[0] != this)
    return;
  ...
  ECSEntityObjectPropertyPanel entityObjectPropertyPanel(*static_cast<ECSBasicObjectEditor *>(getObjEditor()), panel);
  entityObjectPropertyPanel.onPPChange(pid, edit_finished, getEid());
}

void ECSEntityObject::onPPBtnPressed(int pid, PropPanel::ContainerPropertyControl &panel,
  dag::ConstSpan<RenderableEditableObject *> objects)
{
  if (objects.size() != 1 || objects[0] != this)
    return;
  ...
  ECSEntityObjectPropertyPanel entityObjectPropertyPanel(*static_cast<ECSBasicObjectEditor *>(getObjEditor()), panel);
  entityObjectPropertyPanel.onPPBtnPressed(pid, getEid());
}
