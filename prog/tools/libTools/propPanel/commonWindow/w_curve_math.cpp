...
CatmullRomCBTest::CatmullRomCBTest() : controlPoints(tmpmem), fromStartingPos(0, 0)
{
  ...
  CatmullRomCBTest::setLockX(false);
  CatmullRomCBTest::setLockEnds(false);
  CatmullRomCBTest::setCycled(false);
}
...
bool CubicPSplineCB::getCoefs(Tab<Point2> &xy_4c_per_seg) const
{
  ...
  clear_and_resize(yc, nx * 3 + 1);
  clear_and_resize(rab, 7 * (nx + 3) + 1);
