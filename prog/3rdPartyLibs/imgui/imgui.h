...
typedef void* ImTextureID;      // Default: store up to 64-bits (any pointer or integer). A majority of backends are ok with that.
#endif
...
namespace ImGui
{
    ...
    IMGUI_API bool          SetDragDropPayload(const char* type, const void* data, uint64_t sz, ImGuiCond cond = 0);  // type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui internal types. Data is copied and held by imgui. Return true when payload has been accepted.
    ...
    IMGUI_API void          LoadIniSettingsFromMemory(const char* ini_data, uint64_t ini_size=0); // call after CreateContext() and before the first call to NewFrame() to provide .ini data from your own data source.
