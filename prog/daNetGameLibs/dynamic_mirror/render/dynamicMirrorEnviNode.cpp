...
{
  return get_dynamic_mirrors_namespace().registerNode("prepare_envi", DAFG_PP_NODE_SRC, [&mirror_renderer](dafg::Registry registry) {
    registry.executionHas(dafg::SideEffects::External);
    registry.createBlob<OrderingToken>("mirror_skies_prepared_token");
    ...
    return [mirrorActiveHndl, mirrorCameraHndl, &mirror_renderer]() {
      ...
      skies->prepareSkyAndClouds(sky_is_unreachable, dpoint3(cameraData->viewItm.getcol(3)), dpoint3(cameraData->viewItm.getcol(2)), 2,
        nullptr, nullptr, skiesData, cameraData->viewTm, cameraData->projTm, UpdateSky::On, true,
...
dafg::NodeHandle create_dynamic_mirror_envi_node(DynamicMirrorRenderer &mirror_renderer)
{
  return get_dynamic_mirrors_namespace().registerNode("render_envi", DAFG_PP_NODE_SRC, [&mirror_renderer](dafg::Registry registry) {
    ...
    registry.readBlob<OrderingToken>("mirror_skies_prepared_token");
    ...
    return [mirrorActiveHndl, mirrorCameraHndl, &mirror_renderer]() {
      ...
      {
        skies->renderSky(skiesData, cameraData->viewTm, cameraData->projTm, cameraData->persp);
        skies->renderCloudsToTarget(sky_is_unreachable, nullptr, nullptr, skiesData, cameraData->viewTm, cameraData->projTm);
      }
    };
  });
}
