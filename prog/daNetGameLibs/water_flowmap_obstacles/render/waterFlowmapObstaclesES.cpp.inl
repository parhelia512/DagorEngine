...
static void destroy_water_flowmap_obstacles_entity(ecs::EntityManager &manager)
{
  manager.destroyEntity(manager.getSingletonEntity(ECS_HASH("water_flowmap_obstacles")));
}

ECS_TAG(render)
ECS_ON_EVENT(on_appear, EventLevelLoaded, ChangeRenderFeatures)
static void attempt_to_enable_water_flowmap_obstacles_es(const ecs::Event &, ecs::EntityManager &manager, FFTWater &water)
{
  fft_water::WaterFlowmap *waterFlowmap = fft_water::get_flowmap(&water);
  if (!waterFlowmap || !fft_water::is_flowmap_active(&water))
  {
    destroy_water_flowmap_obstacles_entity(manager);
    return;
  }
...
static void disable_water_flowmap_obstacles_es(const ecs::Event &, ecs::EntityManager &manager)
{
  destroy_water_flowmap_obstacles_entity(manager);
}
...
static void update_water_flowmap_obstacles_es(const UpdateStageInfoBeforeRender &evt, ecs::EntityManager &manager, FFTWater &water)
{
  if (evt.dt <= 0.0)
    return;
  ...
  if (waterFlowmap)
  {
    ...
    manager.broadcastEventImmediate(GatherObstaclesForWaterFlowmap(&evtCtx));
  }
}
