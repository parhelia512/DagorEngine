...
#undef USE_VERT
  ...
  append_items(nodesMemory, sizeof(Node));
  ...
  else
  {
    ...
    Node *n = new (nodesMemory.data() + allocatedAt, _NEW_INPLACE) Node;
    n->bsc = Point3::xyz(nodeSphR2);
    n->bsr2 = nodeSphR2.w;
    n->sub1 = (Node *)left;
    n->sub0 = (Node *)right;
...
template <typename FI>
void BuildableStaticSceneRayTracerT<FI>::replacePointers(Node *n)
{
  if (!n) // leaf
    return;
  if (n->isLeaf())
