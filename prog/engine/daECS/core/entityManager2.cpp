  ...
  create_entity_async_cb_t &&creation_cb)
{
  ...
  *(EntityId *__restrict)(chunkData + idInChunk * sizeof(EntityId)) = eid;
  ...
  const uint32_t createdWordsCount = (componentsCount + createdWordSizeBits - 1) / createdWordSizeBits;
...
EntityManager::RequestResources EntityManager::requestResources(EntityId eid, archetype_t oldArchetype, template_t templId,
  const ComponentsInitializer &initializer, RequestResourcesType type, gameres_list_t *out_not_loaded_resources)
{
  ...
  {
    if (out_not_loaded_resources)
      *out_not_loaded_resources = eastl::move(rcb.requestedResources);
  ...
  {
    if (load_gameres_list(rcb.requestedResources))
      return RequestResources::Loaded;
    if (out_not_loaded_resources)
    {
      filter_out_loaded_gameres(rcb.requestedResources, 0);
      *out_not_loaded_resources = eastl::move(rcb.requestedResources);
    }
    return RequestResources::Error;
  }
...
EntityId EntityManager::createEntitySync(template_t templId, ComponentsInitializer &&initializer, ComponentsMap &&map)
{
  ...
  gameres_list_t notLoadedResources;
  const RequestResources result =
    requestResources(eid, INVALID_ARCHETYPE, templId, initializer, RequestResourcesType::SYNC, &notLoadedResources);
  if (DAGOR_UNLIKELY(result != RequestResources::AlreadyLoaded))
  {
    ...
    {
      logerr("sync creation of entity %d eid (%s) failed, %s", eid, getTemplateName(templId),
        format_not_loaded_gameres_message(eid, notLoadedResources).c_str());
...
template_t EntityManager::instantiateTemplate(int id, bool update_queries)
{
  ...
  DemandClearBitArray<framemem_allocator> seenComps(dataComponents.size());
  compEntries.reserve(templ.components.size());
  compEntries.push_back({0, nullptr}); // eid
  // Pre-collect all tracked+replicated component hashes across the template hierarchy once,
  // so we can replace the per-component recursive getRegExpInheritedFlags with binary search.
  dag::RelocatableFixedVector<component_t, 16, true, framemem_allocator> allTracked, allReplicated;
  if (templ.totalComponentFlags())
  {
    getTemplateDB().iterate_template_parents(templ, [&](const Template &p) {
      auto ts = p.trackedSet();
      allTracked.insert(allTracked.end(), ts.begin(), ts.end());
      auto rs = p.replicatedSet();
      allReplicated.insert(allReplicated.end(), rs.begin(), rs.end());
    });
    stlsort::sort(allTracked.begin(), allTracked.end());
    allTracked.erase(eastl::unique(allTracked.begin(), allTracked.end()), allTracked.end());
    stlsort::sort(allReplicated.begin(), allReplicated.end());
    allReplicated.erase(eastl::unique(allReplicated.begin(), allReplicated.end()), allReplicated.end());
  }
  {
    ...
    getTemplateDB().iterate_template_parents(templ, [&](const Template &p) {
      for (auto templateIt = p.getComponentsMap().begin(), e = p.getComponentsMap().end(); templateIt != e; ++templateIt)
      {
        ...
        if (comp == 0)
          continue;
        if (!seenComps.testAndSet(comp))
          continue;
        compEntries.push_back({comp, &templateIt->second});

        if (componentTypes.getTypeInfo(typeIdx).size != 0) // do not process tags
        {
          uint32_t tempFlags = 0;
          if (eastl::binary_search(allTracked.begin(), allTracked.end(), compHash))
            tempFlags |= FLAG_CHANGE_EVENT;
          if (eastl::binary_search(allReplicated.begin(), allReplicated.end(), compHash))
            tempFlags |= FLAG_REPLICATED;
          if (tempFlags)
          {
            ...
            if ((tempFlags & FLAG_REPLICATED) && should_replicate(comp, *this))
              templReplicatedEntries.push_back(comp);
            ...
            if (seenComps.testAndSet(compCopy))
              compEntries.push_back({compCopy, nullptr});
          }
        }
      }
    });
  }
  // Sort compEntries by component_index so createTemplate gets sorted arrays (required by archetype lookup)
  stlsort::sort(compEntries.begin(), compEntries.end(), [](const auto &a, const auto &b) { return a.first < b.first; });
  stlsort::sort(templReplicatedEntries.begin(), templReplicatedEntries.end());
  // Split into parallel arrays for createTemplate
  dag::RelocatableFixedVector<component_index_t, 32, true, framemem_allocator> templComponentsIndices;
  dag::RelocatableFixedVector<const ChildComponent *, 32, true, framemem_allocator> templComponentsData;
  templComponentsIndices.reserve(compEntries.size());
  templComponentsData.reserve(compEntries.size());
  for (auto &e : compEntries)
  {
    templComponentsIndices.push_back(e.first);
    templComponentsData.push_back(e.second);
  }
  ...
  {
    for (auto ri : templReplicatedEntries)
      canBeReplicated.set(ri, true);
  }
  // todo: free framemem here

  if (update_queries && oldArchetypesCount != archetypes.size())
  {
...
template_t EntityManager::templateByName(const char *templ_name, EntityId eid, bool update_queries)
{
  ...
  if (update_queries && DAGOR_UNLIKELY(templId.singleton))
  {
  ...
  if (DAGOR_LIKELY(templId.t != INVALID_TEMPLATE_INDEX))
    return templId.t;
  return instantiateTemplate(id, update_queries);
}
...
void EntityManager::destroyEntityImmediate(EntityId eid, decltype(freeIndices) *findices)
{
  ...
  if (!findices)
    ;
  else if (arch != INVALID_ARCHETYPE)
  {
#if DAECS_EXTENSIVE_CHECKS
    int j = 0;
  ...
  // Rare code path of destroying entity with invalid archetype - make sure that it isn't freed already as there
  ...
  // To consider: may be using some bit or value of `EntityDesc` for that state?
    ...
    logerr("eid %d<%s> index %d is already in freeIndices (occupied with %d)", eid.value(), getEntityTemplateName(eid), idx, *fit);
  ...
  const ComponentsInitializer &initializer)
{
  gameres_list_t notLoadedResources;
  const RequestResources result =
    requestResources(eid, old_archetype, templ, initializer, RequestResourcesType::CHECK_ONLY, &notLoadedResources);
  if (DAGOR_UNLIKELY(result == RequestResources::Error))
  {
    logerr("Creation of entity %d<%s> is not possible, %s", eid, getTemplateName(templ),
      format_not_loaded_gameres_message(eid, notLoadedResources).c_str());
    return false;
  }
  return true;
...
bool EntityManager::createQueuedEntitiesOOL()
{
  ...
  {
    TIME_PROFILE(preBuildTemplates)
    const uint32_t oldArchetypesCount = archetypes.size();

    for (auto &chunk : delayedCreationQueue)
    {
      for (auto &ce : chunk)
      {
        if (ce.isToDestroy() || !ce.eid || ce.templ != INVALID_TEMPLATE_INDEX)
          continue;
        if (entDescs.doesEntityExist(ce.eid))
          ce.templ = templateByName(ce.templateName.c_str(), ce.eid, false);
        else
          ce.eid = EntityId();
      }
    }
