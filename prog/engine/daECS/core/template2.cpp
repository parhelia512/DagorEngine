...
Template::template_hash_t Template::buildHash() const
{
  ...
  combine_hash(hash, mem_hash_fnv1((const char *)sets.get(), (ignoredOfs() + ignoredCount) * sizeof(component_t)));
...
void TemplateDBInfo::addMetaComp(const char *templ_name, const char *comp_name)
{
  if (!metaInfoData || !templ_name || !comp_name)
    return;
  const uint64_t key = mum_hash_step(str_hash_fnv1<64>(templ_name), str_hash_fnv1<64>(comp_name));
