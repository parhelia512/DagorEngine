...
static uint64_t computeQueryRequiredBitmask(const ResolvedQueryDesc &resDesc)
{
  uint64_t bitmask = 0;
  const uint32_t reqEnd = resDesc.getRwCnt() + resDesc.getRoCnt() + resDesc.getRqCnt();
  for (uint32_t i = 0; i < reqEnd; ++i)
  {
    if (resDesc.getOptionalMask()[i])
      continue;
    bitmask |= componentBit(resDesc.getComponents()[i]);
  }
  return bitmask;
}
...
bool EntityManager::resolvePersistentQueryInternal(uint32_t index)
{
  ...
  if (ret != NOT_RESOLVED)
    queryRequiredBitmasks[index] = computeQueryRequiredBitmask(resolvedQueries[index]);
  DEBUG_VERY_VERBOSE_QUERY("set resolved query <%s> to %d", queryDescs[index].getName(), isResolved(index));
  return ret != NOT_RESOLVED;
}

void EntityManager::resolveQueryComponentSizes(uint32_t index)
{
...
EntityManager::ResolvedStatus EntityManager::resolveQuery(uint32_t index, ResolvedStatus currentStatus, ResolvedQueryDesc &resDesc)
{
  ...
  // Fast pre-check: if cached missing component is still unregistered, skip entirely
  const component_t cachedMissing = firstMissingComponent[index];
  if (cachedMissing != 0 && dataComponents.findComponentId(cachedMissing) == INVALID_COMPONENT_INDEX)
    return NOT_RESOLVED;
  ...
  auto makeRange = [&](dag::ConstSpan<ComponentDesc> components, ReqType req) -> bool {
    ...
    for (auto cdI = components.begin(), cdE = components.end(); cdI != cdE; ++cdI, ++componentsIterated)
    {
      ...
      {
        if (req == REQUIRED_NOT || (cd.flags & CDF_OPTIONAL)) // optional component can be even not registered at all, but it is not
                                                              // fully resolved then
        {
          ret &= ~FULLY_RESOLVED;
          continue;
        }
        firstMissingComponent[index] = cd.name;
        ret = NOT_RESOLVED;
        return false;
      }
      // verify type
  ...
  if (!wasResolved)
  {
    ...
    const bool firstAttempt = resDesc.getComponents().size() == 0;
    DAECS_EXT_ASSERT(firstAttempt || resDesc.getComponents().size() == totalComponents);
    if (firstAttempt)
    {
    ...
    // reduce finds in later stages - skip already-found components
    for (uint32_t i = 0, e = checkComponents; i != e; ++i)
    {
      if (resDesc.getComponents()[i] != INVALID_COMPONENT_INDEX)
        continue;
...
static T *add_to_fixed_container(T *ft, Cnt &__restrict cnt, const T *__restrict add, size_t size)
{
  ...
  {
    const size_t allocSize = max(nextSize, size_t(cnt) * 3 / 2); // 1.5x growth to amortize repeated appends
    T *next = (T *)memrealloc_default(ft, allocSize * sizeof(T));
    if (DAGOR_UNLIKELY(next == nullptr))
    {
      next = (T *)memalloc_default(allocSize * sizeof(T)); // so no mem setting
...
bool EntityManager::makeArchetypesQuery(archetype_t first_archetype, uint32_t index, QueryApplyMode mode) //, bool
                                                                                                          // componentsCountChanged
{
  ...
  if (updateOneArchetype && !fitsAll) // full pre-check before FRAMEMEM_REGION - avoids framemem alloc on mismatch
  {
    ...
    const uint64_t qbm = queryRequiredBitmasks[index];
    if (qbm != 0 && (archetypes.getBloomBitmask(ai) & qbm) != qbm)
      return relatedArchsRet;
    ...
    {
      if (archInfo.getComponentId(resDesc.getComponents()[i]) == INVALID_ARCHETYPE_COMPONENT_ID && !resDesc.getOptionalMask()[i])
        return relatedArchsRet;
    }
  }
...
bool EntityManager::makeArchetypesQueryInternal(archetype_t last_arch_count, uint32_t index, bool should_re_resolve)
{
  ...
  // Use status captured before resolve - if query was NOT fully resolved before, it must enter the setup branch
  // (PartiallyResolved) even if resolve just succeeded, because query component counts haven't been populated yet
  return makeArchetypesQuery(last_arch_count, index,
    isFullyResolved(status) ? QueryApplyMode::FullyResolved : QueryApplyMode::PartiallyResolved);
}
...
void EntityManager::invalidatePersistentQueries()
{
  ...
  for (size_t i = 0, e = archetypeQueries.size(); i < e; ++i)
  {
    ...
    queryRequiredBitmasks[i] = 0;
    firstMissingComponent[i] = 0;
  }
...
uint32_t EntityManager::addOneQuery()
{
  ...
  queryRequiredBitmasks.push_back(0);
  firstMissingComponent.push_back(0);
...
inline EntityManager::query_components_hash EntityManager::query_components_hash_calc(const BaseQueryDesc &desc)
{
  ...
  wyhash_state s = wyhash_init(0);
  wyhash_add(s, &details, sizeof(details));
  auto hashComponents = [&s](dag::ConstSpan<ComponentDesc> list) {
    for (const auto &c : list)
    {
      struct
      {
        ...
        uint8_t flags;
      } key = {c.name, c.type, (uint8_t)c.flags};
      wyhash_add(s, &key, sizeof(component_t) + sizeof(component_type_t) + 1);
    }
  };
...
QueryId EntityManager::createQuery(const NamedQueryDesc &desc)
{
  ...
  if (queriesReferences[index] == 1 && resolvePersistentQueryInternal(index)) // recently added
    makeArchetypesQuery(0, index, QueryApplyMode::PartiallyResolved);         // first call - must set up query component counts
  return h;
}
...
void EntityManager::updateAllQueriesInternal()
{
  ...
  for (uint32_t ai = allQueriesUpdatedToArch, ae = archetypes.size(); ai < ae; ++ai)
    newArchsBitmask |= archetypes.getBloomBitmask(ai);
  for (int index = 0, e = queriesReferences.size(); index < e; ++index)
  {
    if (!queriesReferences[index])
      continue;
    // When no new components were registered, NOT_RESOLVED queries can't possibly resolve - skip them
    if (!shouldResolveQueries && getQueryStatus(index) == NOT_RESOLVED)
      continue;
    const uint64_t qbm = queryRequiredBitmasks[index];
    if (qbm != 0 && (newArchsBitmask & qbm) != qbm)
    {
      // Bloom filter says new archetypes can't match this query's required components.
      // But if new component types were registered and query is not fully resolved,
      // we must still attempt re-resolution and update component sizes (e.g. REQUIRE_NOT
      // components that were unknown at query creation time may now be resolvable).
      if (shouldResolveQueries && !isFullyResolved(getQueryStatus(index)))
      {
        resolvePersistentQueryInternal(index);
        resolveQueryComponentSizes(index);
      }
      continue;
    }
    if (updatePersistentQueryInternal(allQueriesUpdatedToArch, index, shouldResolveQueries))
    {
  ...
  // currently all queries are up-to-date by the time validateQuery is called;
  {
...
#if DAECS_EXTENSIVE_CHECKS && VALIDATE_QUERY_THOROUGHLY // very expensive check
    {
      ...
      if (isResolved(status))
      {
        ...
        if (!makeArchetypesQuery(0, index, QueryApplyMode::PartiallyResolved))
        {
