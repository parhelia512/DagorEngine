...
#include <daECS/core/tokenize_const_string.h> // FIXME
...
static void create_entities_blk_internal(ecs::EntityManager &mgr, const DataBlock &blk, const char *blk_path,
  const on_entity_creation_scheduled_t &on_creation_scheduled_cb, const create_entity_async_cb_t &on_entity_created_cb,
  const on_import_beginend_cb_t &on_import_beginend_cb, const on_scene_blk_loaded_cb_t &on_scene_blk_loaded_cb, TMatrix tm,
  bool is_import)
{
  int enid = blk.getNameId("entity"), tnid = blk.getNameId("_template"), impnid = blk.getNameId("import"),
      scnid = blk.getNameId("scene"), scntmid = blk.getNameId("transform");
  ...
  if (on_scene_blk_loaded_cb)
  {
    on_scene_blk_loaded_cb(blk);
  }
  ...
  for (int i = 0; i < blk.blockCount(); ++i)
    if (blk.getBlock(i)->getBlockNameId() == enid)
    {
      ...
      if (is_import) // apply parent transform of the import
      {
        if (auto it = cmap.find_as(ECS_HASH("transform")); it != cmap.end())
        {
          TMatrix &eTm = it->second.getRW<TMatrix>();
          eTm = tm * eTm;
        }
      }
    ...
    else if (blk.getBlock(i)->getBlockNameId() == impnid)
    {
      ...
      const auto loadAndCreateEntities = [&](const char *import_scene, TMatrix scene_tm) {
        if (on_import_beginend_cb)
          on_import_beginend_cb(import_scene, &scene_tm, /*begin*/ true);

        String importPathBuffer;
        const char *importScene = import_scene;
        if (dd_file_exist(importScene) ||
            (resolve_import_path && dd_file_exist(importScene = resolve_import_path(importScene, importPathBuffer))))
        {
          DataBlock importBlk;
          if (dblk::load(importBlk, importScene, dblk::ReadFlag::ROBUST_IN_REL)) // not ROBUST in dev to see syntax errors
          {
            create_entities_blk_internal(mgr, importBlk, import_scene, on_creation_scheduled_cb, on_entity_created_cb,
              on_import_beginend_cb, on_scene_blk_loaded_cb, tm * scene_tm, true);
          }
        }
        else
          logerr("Missing import scene <%s> in <%s>", import_scene, blk_path);

        if (on_import_beginend_cb)
          on_import_beginend_cb(import_scene, &scene_tm, /*begin*/ false);
      };

      if (const char *importScene = iblk.getStrByNameId(scnid, nullptr))
      {
        const TMatrix sceneTm = iblk.getTmByNameId(scntmid, tm);
        loadAndCreateEntities(importScene, sceneTm);
      }
    }
    else
...
void BlkLoadContext::load(const DataBlock &blk, const char *tags, on_empty_comp_type_cb_t on_empty_comp_type_cb)
{
  ...
  for (int j = 0, e = blk.blockCount(); j < e; ++j)
  {
    ...
    const bool doLoad = !ftags || (info && ecs::filter_needed(ftags, info->filterTags));
...
{
  create_entities_blk_internal(mgr, blk, blk_path, on_creation_scheduled_cb, on_entity_created_cb, on_import_beginend_cb,
    on_scene_blk_loaded_cb, TMatrix::IDENT, false);
}
...
eastl::function<const char *(const char *, String &)> resolve_import_path = {};
