...
bool write_server_eid(ecs::entity_id_t eidVal, danet::BitStream &bs, dag::FunctionRef<eastl::string() const> ctx_err_cb)
{
  ...
  uint32_t generation = ecs::get_generation(eid);
...
int Connection::prepareWritePackets()
{
  ...
  for (int i = numDirtyReplicas - 1; i >= 0; --i) // reverse iteration to be able remove (freeObjectReplica)
  {
    ...
    // mark for kill replicas that wasn't added in scope and not yet marked for kill
    if (!(repl->flags & (ObjectReplica::InScope | ObjectReplica::ToKill)))
...
void Connection::writeClientReplayKeyFrame(danet::BitStream &bs, danet::BitStream &tmpbs, dag::ConstSpan<ecs::EntityId> eids)
{
  ...
  // Step 2. Sync all entities as fresh entities
  {
    TIME_PROFILE(ecs_replay_keyframe__entities)
    BitSize_t blockSizePos = bs.GetWriteOffset();
    uint16_t serializeBlockCount = 0;
    bs.Write(serializeBlockCount);
  ...
  // Step 4. Serialize exist network templates
  {
    TIME_PROFILE(ecs_replay_keyframe__srv_templates)
    dag::Vector<ecs::component_index_t, framemem_allocator> clientToServerCidx;
    for (ecs::component_index_t serverCidx = 0; serverCidx < serverToClientCidx.size(); serverCidx++)
    {
      const ecs::component_index_t clientCidx = serverToClientCidx[serverCidx];
      if (clientCidx == ecs::INVALID_COMPONENT_INDEX)
        continue;
      if (clientToServerCidx.size() <= clientCidx)
        clientToServerCidx.resize(clientCidx + 1, ecs::INVALID_COMPONENT_INDEX);
      clientToServerCidx[clientCidx] = serverCidx;
    }
    eastl::bitvector<> componentsSyncedTmp;
    BitSize_t blockSizePos = bs.GetWriteOffset();
    uint16_t serializeBlockCount = 0;
    ...
    {
      serializeTemplateForClientReplay(bs, ecs::template_t(i), componentsSyncedTmp, clientToServerCidx);
      ++serializeBlockCount;
    }
    bs.WriteAt(serializeBlockCount, blockSizePos);
  }
  ...
  // Step 5. Serialize string to index
  {
    TIME_PROFILE(ecs_replay_keyframe__strings)
    BitSize_t blockSizePos = bs.GetWriteOffset();
    uint16_t serializeBlockCount = 0;
    bs.Write(serializeBlockCount);
    for (auto const &it : objectKeysRepl.shared->index)
