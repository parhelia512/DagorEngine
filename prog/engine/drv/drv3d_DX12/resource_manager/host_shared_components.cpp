...
{
eastl::optional<HostDeviceSharedMemoryRegion> FramePushRingMemoryProvider::allocatePushMemory(DXGIAdapter *adapter, Device &device,
  uint32_t size, uint32_t alignment, uint32_t frame_index)
{
  // Large allocations skip the push ring to avoid wasting ring space.
  if (size > largePushThreshold)
  {
    return {};
  }
  G_UNUSED(adapter);
  G_UNUSED(device);
  auto result = pushRing.access()->allocate(size, alignment, frame_index);
  if (result)
  {
...
TemporaryUploadMemoryProvider::HostDeviceSharedMemoryRegionAllocationResult TemporaryUploadMemoryProvider::allocatePushMemory(
  DXGIAdapter *adapter, Device &device, uint32_t size, uint32_t alignment, uint32_t frame_index)
{
  auto result = BaseType::allocatePushMemory(adapter, device, size, alignment, frame_index);
  if (result)
  {
    return *result;
  }

  if (!device.DeviceErrorState::checkResourceCreateState("allocatePushMemory"))
  {
    return {};
  }

  const auto properties =
    getProperties(D3D12_RESOURCE_FLAG_NONE, TemporaryUploadMemoryInfo::memory_class, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT);
...
TemporaryUploadMemoryProvider::HostDeviceSharedMemoryRegionAllocationResult TemporaryUploadMemoryProvider::allocateTempUpload(
...
PersistentUploadMemoryProvider::HostDeviceSharedMemoryRegionAllocationResult PersistentUploadMemoryProvider::
  allocatePersistentUploadMemory(DXGIAdapter *adapter, Device &device, size_t size, size_t alignment)
{
  ...
  if (checkForOOM(adapter, as_optional_error(result),
...
PersistentReadBackMemoryProvider::HostDeviceSharedMemoryRegionAllocationResult PersistentReadBackMemoryProvider::
  allocatePersistentReadBack(DXGIAdapter *adapter, Device &device, size_t size, size_t alignment)
{
...
PersistentBidirectionalMemoryProvider::HostDeviceSharedMemoryRegionAllocationResult PersistentBidirectionalMemoryProvider::
  allocatePersistentBidirectional(DXGIAdapter *adapter, Device &device, size_t size, size_t alignment)
{
  ...
  if (checkForOOM(adapter, as_optional_error(result),
        OomReportData{"allocatePersistentBidirectional", nullptr, size, AllocationFlags{}.toUlong(), properties.raw}))
  {
