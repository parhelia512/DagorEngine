...
void BEContext::syncConstDepthReadWithInternalStore()
{
...
void BEContext::ensureStateForColorAttachments(VkRect2D area)
{
...
void BEContext::ensureStateForShadingRateAttachment()
{
#if VK_KHR_fragment_shading_rate
  const RenderPassClass::FramebufferDescription::AttachmentInfo &srAtt = getFramebufferState().frameBufferInfo.shadingRateAttachment;
  D3D_CONTRACT_ASSERTF(srAtt.img,
    "vulkan: no shading rate attachment specified for render pass class with used shading rate attachment");
  verifyResident(srAtt.img);
  srAtt.img->checkDead();

  Backend::sync.addImageAccess(LogicAddress::forAttachmentWithLayout(VK_IMAGE_LAYOUT_FRAGMENT_SHADING_RATE_ATTACHMENT_OPTIMAL_KHR),
    srAtt.img, VK_IMAGE_LAYOUT_FRAGMENT_SHADING_RATE_ATTACHMENT_OPTIMAL_KHR,
    {srAtt.view.getMipBase(), srAtt.view.getMipCount(), srAtt.view.getArrayBase(), srAtt.view.getArrayCount()});
#endif
}

void BEContext::ensureStateForNativeRPDepthAttachment()
{
...
void BEContext::ensureStateForDepthAttachment(VkRect2D area)
{
...
void BEContext::beginPassInternal(RenderPassClass *pass_class, VulkanFramebufferHandle fb_handle, VkRect2D area)
{
  ...
  StaticTab<VkImageView, Driver3dRenderTarget::MAX_SIMRT + MRT_EXTRA_SLOTS> attachments;
  if (Globals::cfg.has.imagelessFramebuffer)
  {
    attachments = passIdent.squash<VkImageView>(eastl::size(fbs.frameBufferInfo.colorAttachments),
      fbs.frameBufferInfo.colorAttachments, fbs.frameBufferInfo.depthStencilAttachment, fbs.frameBufferInfo.shadingRateAttachment,
...
void BEContext::endPass(const char *why)
{
...
void BEContext::nextNativeSubpass()
{
#if DAGOR_DBGLEVEL > 0
  InPassStateFieldType inPassState = Backend::State::exec.get<StateFieldGraphicsInPass, InPassStateFieldType, BackGraphicsState>();
...
void BEContext::beginNativePass()
{
#if DAGOR_DBGLEVEL > 0
  InPassStateFieldType inPassState = Backend::State::exec.get<StateFieldGraphicsInPass, InPassStateFieldType, BackGraphicsState>();
...
void BEContext::processBufferCopyReorders(uint32_t pass_index)
{
...
void BEContext::finishReorderAndPerformSync()
{
...
void BEContext::interruptFrameCoreForRenderPassStart()
{
