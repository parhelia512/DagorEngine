...
static bool prepare_file_impl(const char *path, write_stream_t &fpout, int lev_bit)
{
  ...
  dd_mkpath(path);
  if (lastDbgPath[0] && strlen(dbgFilepath) > 5)
  {
...
void debug_internal::vlog(int lev, const char *fmt, const void *arg, int anum)
{
...
void start_classic_debug_system(const char *debug_fname, bool en_time, bool hhmmss_fmt, int hhmmss_subsec, bool append_file)
{
  ...
  if (last_char_ptr && debug_fname && last_char_ptr > debug_fname + 1 && strcmp(last_char_ptr - 1, "/#") == 0)
  {
#if DAGOR_FORCE_LOGS
    // crypted logs
