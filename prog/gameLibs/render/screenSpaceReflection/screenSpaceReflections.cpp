...
void ScreenSpaceReflections::updatePrevTarget()
{
  ...
  d3d::update_sub_region(ssrTarget->getTex2D(), 0, 0, 0, 0, ssrW, ssrH, 1, ssrPrevTarget->getTex2D(), 0, 0, 0, 0);
}

void ScreenSpaceReflections::setHistoryValid(bool valid) { isHistoryValid = valid; }

ScreenSpaceReflections::Reprojection ScreenSpaceReflections::getReprojection() const
{
  Reprojection reprojection;
  reprojection.afr = afrs.current();
  return reprojection;
}

void ScreenSpaceReflections::applyReprojection(const Reprojection &reprojection, const TMatrix &view_tm, const TMatrix4 &proj_tm,
  const DPoint3 &world_pos)
{
  Afr afr = reprojection.afr;
  ...
  SubFrameSample sub_sample, BaseTexture *ssr_tex, BaseTexture *ssr_prev_tex, BaseTexture *target_tex, int callId)
{
  ...
  if (!hasPrevTarget && ownTextures)
  {
    ...
    d3d::update_sub_region(target_tex, 0, 0, 0, 0, ssrW, ssrH, 1, ssr_tex, 0, 0, 0, 0);
  }
