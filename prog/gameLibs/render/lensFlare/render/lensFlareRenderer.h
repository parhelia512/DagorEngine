...
struct LensFlareConfig
{
  ...
  // Moves the light source closer to the camera before depth compare.
  // Only relevant if useOcclusion=true
  float depthBias = 0;
...
private:
  ...
  public:
    ...
    static LensFlareData parse_lens_flare_config(const LensFlareConfig &config, eastl::vector<uint16_t> &ib,
      eastl::vector<LensFlareVertex> &vb, eastl::vector<LensFlareInfo> &flares, eastl::vector<Point2> &vpos);
  ...
  static constexpr int MAX_NUM_MANUAL_PREPARED_LIGHTS = 64;
  ...
  UniqueBuf indirectDrawStagingBuf;
  UniqueBuf indirectDrawInstanceOffsetsBuf; // Stored separately to ensure consistent behaviour between 3d apis

  // Used for occlusion test right after the prepare shader, runs on pre-culled instances
  UniqueBuf indirectDispatchInitialBuf; // Initial value, the other buffer is reset to this on every frame
  ...
  // This buffers stores visibility history (provided by the occlusion shader) of lens flares
  // The hash of the position of flare sources (lights) are used as indices.
  // Only visible flares use it. It needs to hold instances visible at once at any given time.
  // Index collisions (when two different hashes are mapped to the same index) are handled by invalidating the previous cache =
