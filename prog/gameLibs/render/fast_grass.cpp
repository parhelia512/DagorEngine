...
void FastGrassRenderer::initOrUpdate(dag::Span<GrassTypeDesc> grass_types, int dim_bits)
{
#define VAR(a) a = ShaderVariableInfo(#a, true).get_var_id();
  SHADER_VARS_LIST
#undef VAR
#define VAR(a) a = VariableMap::getVariableId(#a);
  SHADER_OPT_VARS_LIST
#undef VAR
  ...
  if (!grassChannelsCB)
  {
    grassChannelsCB = dag::buffers::create_persistent_cb(dag::buffers::cb_array_reg_count<FastGrassType>(GRASS_MAX_CHANNELS + 1),
      "fast_grass_types_buf", RESTAG_GRASS);
    if (!grassChannelsCB)
    {
  ...
  if (precompCascades > 0 && precompResolution > 0)
  {
    precompShader = ComputeShader("fast_grass_precomp");
    ...
    if (!clipmapRectsCB)
    {
      clipmapRectsCB = dag::buffers::create_persistent_cb(
        dag::buffers::cb_array_reg_count<Point4>(FAST_GRASS_MAX_CLIPMAP_CASCADES * 2 + 1), "fast_grass_clip_rects", RESTAG_GRASS);
      if (!clipmapRectsCB)
      {
        logerr("fast_grass: failed to create buffer");
        return;
...
FastGrassRenderer::FastGrassRenderer() {}
...
void FastGrassRenderer::prepare(PreparedData &pd, const TMatrix4 &globtm, HeightmapHandler &handler,
  const HeightmapFrustumCullingInfo &input_fi)
{
  pd.valid = false;
  ...
  if (!alreadyLoaded)
  {
    if (auto tex = normalTex.getBaseTex())
    {
      TextureInfo info;
      tex->getinfo(info);
      ShaderGlobal::set_float(fast_grass_aspect_ratio, float(info.w) / info.h);
    }
    alreadyLoaded = true;
  }
  ...
  TIME_D3D_PROFILE(prepare_fast_grass);

  HeightmapFrustumCullingInfo fi = input_fi;
  Frustum frustum{fi.frustum};
  const Point3 view_pos = fi.world_pos;
  ...
  handler.frustumCulling(pd.cullData, fi);
  if (!pd.cullData.hasPatches())
    return;

  int numClips = 0;

  if (!clipmapScales.empty())
  {
  ...
  ShaderGlobal::set_float(fast_grass_slice_step, sliceStep);
  ShaderGlobal::set_float(fast_grass_fade_start, fadeStart);
  ShaderGlobal::set_float(fast_grass_fade_range, max(fadeEnd - fadeStart, 1e-6f));
  ShaderGlobal::set_float(fast_grass_step_scale, stepScale);
  ShaderGlobal::set_float(fast_grass_height_variance_scale, heightVarianceScale);
  ShaderGlobal::set_float(fast_grass_smoothness_fade_start, smoothnessFadeStart);
  ShaderGlobal::set_float(fast_grass_smoothness_fade_end, smoothnessFadeEnd);
  ShaderGlobal::set_float(fast_grass_normal_fade_start, normalFadeStart);
  ShaderGlobal::set_float(fast_grass_normal_fade_end, normalFadeEnd);
  ShaderGlobal::set_float(fast_grass_placed_fade_start, placedFadeStart);
  ShaderGlobal::set_float(fast_grass_placed_fade_end, placedFadeEnd);
  ShaderGlobal::set_float(fast_grass_ao_max, 1 - aoMaxStrength);
  ShaderGlobal::set_float(fast_grass_ao_curve, aoCurve);
...
void FastGrassRenderer::render(const PreparedData &pd, HeightmapHandler &handler)
{
  if (!pd.valid)
    return;
  TIME_D3D_PROFILE(render_fast_grass);
  handler.setVars();
  set_globtm_to_shader(pd.globtm);
  hmapRenderer.render(pd.cullData, hmapRenderer.getShElem(), hmapRenderer.getDimBits());
}
