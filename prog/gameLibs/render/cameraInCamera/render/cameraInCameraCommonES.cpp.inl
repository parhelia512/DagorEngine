...
template <typename Callable>
static inline void get_camcam_viewport_scissor_ecs_query(ecs::EntityManager &manager, Callable);
...
static void process_lens_activation_state(const bool feature_enabled_for_scope, bool &camcam__lens_render_active, int &camcam__iFrame,
  bool &camcam__frame_after_deactivation, const AimRenderingData &aim_data)
{
...
ViewportEllipse calc_viewport_ellipse(const float lens_bounding_sphere_radius, const TMatrix lens_wtm,
  const TMatrix4 view_rot_jitter_proj_tm, float viewport_scale)
{
...
Point4 calc_viewport_uv_rect(const Point2 &center, const Point2 &half_extent, const float viewport_scale)
{
  const Point2 ellipseSpaceScale{1.0f, viewport_scale};
  const Point2 xy = div(center - half_extent, ellipseSpaceScale);
  const Point2 wh = div(half_extent * 2.0f, ellipseSpaceScale);
  return Point4{xy.x, xy.y, wh.x, wh.y};
}

void set_lens_area_scissor()
{
  int vx, vy, vw, vh;
  float vn, vf;
  d3d::getview(vx, vy, vw, vh, vn, vf);
  get_camcam_viewport_scissor_ecs_query(*g_entity_mgr, [&](const Point4 &camcam__lens_viewport_uv) {
    const Point4 &r = camcam__lens_viewport_uv;
    const int x0 = clamp(vx + (int)floorf(r.x * vw), vx, vx + vw);
    const int y0 = clamp(vy + (int)floorf(r.y * vh), vy, vy + vh);
    const int x1 = clamp(vx + (int)ceilf((r.x + r.z) * vw), vx, vx + vw);
    const int y1 = clamp(vy + (int)ceilf((r.y + r.w) * vh), vy, vy + vh);
    if (x1 > x0 && y1 > y0)
      d3d::setscissor(x0, y0, x1 - x0, y1 - y0);
  });
}

} // namespace

void update_transforms(const CameraParams &main_view, const CameraParams &cockpit_view, const CameraParams &prev_cockpit_view,
  const CameraParams &lens_view)
{
  const AimRenderingData aimData = get_aim_rendering_data();
  ...
  const Point4 lensAreaViewportUV = calc_viewport_uv_rect(curFrameEllipse.center, curFrameEllipse.halfExtent, viewportScale);

  update_camcam_transforms_ecs_query(*g_entity_mgr,
    [&uvRemapping, &lensAreaViewportUV](Point4 &camcam__uv_remapping, Point4 &camcam__lens_viewport_uv) {
      camcam__uv_remapping = uvRemapping;
      camcam__lens_viewport_uv = lensAreaViewportUV;
    });
}
...
static const Point4 IDENTITY_UV_REMAPPING = Point4(1.0f, 0.0f, 1.0f, 0.0f);

static void set_uv_remapping_shvar(const bool is_main_view)
{
  Point4 defVal = IDENTITY_UV_REMAPPING;
  if (!is_main_view)
  {
...
static void reset_uv_remapping_shvar() { ShaderGlobal::set_float4(var::camera_in_camera_vp_uv_remapping, IDENTITY_UV_REMAPPING); }
...
ApplyMasterState::ApplyMasterState(const bool is_main_view, const OpaqueFlags flags)
{
  ...
  hasScissor = isCamCamRenderActive && !is_main_view;
  ...
  if (hasStencilTest || hasScissor)
  {
    shaders::OverrideState st = hasStencilTest ? get_stencil_test_override_state() : shaders::OverrideState{};
    if (hasScissor)
      st.set(shaders::OverrideState::SCISSOR_ENABLED);
    shaders::overrides::set_master_state(st);
  }

  if (hasScissor)
    set_lens_area_scissor();
}
...
ApplyPostfxState::ApplyPostfxState(const bool is_main_view, const CameraParams &view, const bool use_stencil)
{
  ...
  const TMatrix4 projtm = view.jitterProjTm.transpose();
  savedProjtmPsf0 = replace_color4(var::projtm_psf_0, Color4(projtm[0]));
  savedProjtmPsf1 = replace_color4(var::projtm_psf_1, Color4(projtm[1]));
  savedProjtmPsf2 = replace_color4(var::projtm_psf_2, Color4(projtm[2]));
  savedProjtmPsf3 = replace_color4(var::projtm_psf_3, Color4(projtm[3]));
...
ApplyPostfxState::ApplyPostfxState(const dafg::multiplexing::Index &multiplexing_index, const CameraParams &view,
...
ApplyPostfxState::ApplyPostfxState(const bool is_main_view, const CameraParams &cur_view, const CameraParams &prev_view,
  const bool use_stencil) :
...
ApplyPostfxState::ApplyPostfxState(const dafg::multiplexing::Index &multiplexing_index, const CameraParams &cur_view,
  const CameraParams &prev_view, bool use_stencil) :
