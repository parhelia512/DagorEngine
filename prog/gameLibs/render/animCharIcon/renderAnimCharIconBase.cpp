...
#include <render/dag_renderAnimCharIconBase.h>
...
void IconAnimchar::init(AnimV20::IAnimCharacter2 *animchar_, const DataBlock *blk_)
{
  ...
  parentIndex = blk->getInt("parentIndex", 0);
  ...
  float &centerY)
{
  ...
  {
    // Update every attachment's transform first: a child positions against its parent, which must
    // be current even if the parent itself does not participate in the bbox (calcBBox=false).
    iconAnimchar[i].updateTmWtm(iconAnimchar[iconAnimchar[i].parentIndex].animchar);
...
// Attachments are appended parent-before-child, so a valid parentIndex is in [0, self_index).
// Anything else (forward or self reference) is bad data; fall back to the base weapon (0).
static int validate_parent_index(int parent_index, int self_index, const DataBlock *blk)
{
  if (parent_index < 0 || parent_index >= self_index)
  {
    logerr("[AnimCharIcon] attachment '%s' has invalid parentIndex=%d (own index %d); using weapon root",
      blk ? blk->getStr("animchar", "?") : "?", parent_index, self_index);
    return 0;
  }
  return parent_index;
}
...
static AsynLoadAnimcharResult sync_load_animchar_with_attachments(const DataBlock &blk, IconAnimcharWithAttachments &res)
{
  ...
  for (int i = 0; i < attachmentsBlk->blockCount(); ++i)
  {
    ...
    res[i + 1].parentIndex = validate_parent_index(res[i + 1].parentIndex, i + 1, aBlk);
...
RenderAnimCharIconBase::AnimcharPrefetch RenderAnimCharIconBase::prepareAnimchar(const PictureManager::PictureRenderContext &pic_ctx)
{
  ...
  if (iconAnimchar.empty())
  {
    ...
    for (auto &ia : iconAnimchar)
    {
      ...
      const DataBlock *shaderChangeTex = ia.blk->getBlockByNameEx("shaderChangeTex");
      if (!shaderChangeTex->isEmpty())
      {
        for (int i = 0; i < shaderChangeTex->blockCount(); ++i)
        {
          eastl::vector<eastl::pair<int, TEXTUREID>, framemem_allocator> tex_names;
          const DataBlock *ruleBlock = shaderChangeTex->getBlock(i);
          const char *shaderName = ruleBlock->getBlockName();
          for (int j = 0; j < ruleBlock->paramCount(); ++j)
          {
            const char *shaderParamName = ruleBlock->getParamName(j);
            const char *shaderTexName = ruleBlock->getStr(j);
            const int varId = get_shader_variable_id(shaderParamName, true);
            if (varId >= 0)
              tex_names.emplace_back(varId, dag::get_tex_gameres(shaderTexName).getTexId());
          }
          if (!tex_names.empty())
          {
            recreate_material_with_new_params(ia.animchar->rendComp(), shaderName, [&](ShaderMaterial *mat) {
              for (const eastl::pair<int, TEXTUREID> &tex : tex_names)
                mat->set_texture_param(tex.first, tex.second);
            });
          }
        }
      }
  ...
  const PictureManager::PictureRenderContext &pic_ctx, const IconSSAA &icon_ssaa)
{
  ...
  for (size_t i = 1; i < render_ctx.iconAnimchars.size(); ++i)
    render_ctx.iconAnimchars[i].updateTmWtm(render_ctx.iconAnimchars[render_ctx.iconAnimchars[i].parentIndex].animchar);
