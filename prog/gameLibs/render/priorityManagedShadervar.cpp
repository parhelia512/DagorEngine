...
PriorityManagedShadervar::ShadervarUnion PriorityManagedShadervar::getBase() { return values.empty() ? get() : defaultValue; }
...
{
  auto base = map.getEntry(id).getBase();
  const float *v = eastl::get_if<float>(&base);
  G_ASSERT_RETURN(v, 0.f);
  return *v;
}
int get_base_int(int id)
{
  auto base = map.getEntry(id).getBase();
  const int *v = eastl::get_if<int>(&base);
  G_ASSERT_RETURN(v, 0);
  return *v;
}
...
{
  auto base = map.getEntry(id).getBase();
  const Point4 *v = eastl::get_if<Point4>(&base);
  G_ASSERT_RETURN(v, Point4::ZERO);
  return *v;
}
IPoint4 get_base_int4(int id)
{
  auto base = map.getEntry(id).getBase();
  const IPoint4 *v = eastl::get_if<IPoint4>(&base);
  G_ASSERT_RETURN(v, IPoint4::ZERO);
  return *v;
}
} // namespace PriorityShadervar
