...
#include <render/puddle_query/shaders/puddle_query.hlsli>
...
{
  if (!shader_exists("puddle_query_cs"))
  {
    debug("PuddleQuery: shader 'puddle_query_cs' is not present, initialization skipped");
    return;
  }
  ...
  {
    logerr("PuddleQuery: Could not find shader variable \"puddle_results_offset_no\"");
    return;
  }
  int numInputsSlot = -1;
