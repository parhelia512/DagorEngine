...
static constexpr float max_ri_phys_rad = 5.f;
...
// One material of the node, preferring a non-default one, or PHYSMAT_INVALID when it has none.
// A fused node's set is in coverage order, so what a destructable adopts follows the material covering most faces AMONG the
// non-default ones, not the exporter's node split.
static PhysMat::MatID get_preferred_node_physmat(const CollisionResource &coll_res, const CollisionNode &node)
{
  PhysMat::MatID matId = PHYSMAT_INVALID;
  for (int i = 0, e = coll_res.getNodePhysMatCount(node.nodeIndex); i < e; ++i)
  {
    const PhysMat::MatID nodeMat = coll_res.getNodePhysMatId(node.nodeIndex, i);
    if (nodeMat < 0)
      continue;
    matId = nodeMat;
  ...
  if (const CollisionResource *collRes = rendinst::getRiGenCollisionResource(desc))
    for (const CollisionNode &node : collRes->getAllNodes())
    {
      if (!collRes->checkNodeBehaviorFlags(node.nodeIndex, CollisionNode::PHYS_COLLIDABLE))
        continue;
      const PhysMat::MatID nodeMat = get_preferred_node_physmat(*collRes, node);
      if (nodeMat < 0)
        continue;
      matId = nodeMat;
      if (matId != PHYSMAT_DEFAULT)
...
static void fill_destr_obj_params(destructables::DestructableCreationParams &params, const rendinstdestr::DestrObjProps *obj)
{
  static const rendinstdestr::DestrObjProps default_obj;
  if (!obj)
    obj = &default_obj;
  params.timeToLive = obj->timeToLive;
  params.inactiveTimeBeforeSink = obj->inactiveTimeBeforeSink;
  params.timeToKinematic = obj->timeToKinematic;
  params.timeToSinkUnderground = obj->timeToSinkUnderground;
  params.timeToStartDisintegration = obj->timeToStartDisintegration;
  params.disintegrationDuration = obj->disintegrationDuration;
  params.disintegrationScale = obj->disintegrationScale;
  params.keepAlive = obj->keepAlive;
  params.interactiveDebris = obj->interactiveDebris;
}
...
struct AsyncFractureJob : cpujobs::IJob
{
  ...
  const rendinstdestr::RiDestrProps *props = nullptr;
  frx::ShatterImpactProfile shatterImpactProfile;
  Point3 impactImpulse = Point3::ZERO;
  PhysMat::MatID physMatId = PHYSMAT_DEFAULT;

  const char *getJobName(bool &) const override { return DAPROFILER_STRING("AsyncFractureJob"); }
  ...
  void doAsyncFracture()
  {
    ...
    int16_t interiorMatId = -1;
    if (const rendinstdestr::RiDestrMaterial *interiorMat = props->frxDestr->interiorMat)
    ...
    frx::DestrSystem sys;
    {
      frx::mesh_slice(ctx, riMesh,
        {
          .upMesh = &sys.pieces[0],
          .cutPlane = frx::PlaneBasis(Plane3(Point3(0, 1, 0), riMesh.tm.getcol(3) + Point3(0, 0.05f, 0))),
          .cutMatId = interiorMatId,
        });
      if (sys.pieces[0].faces.empty())
        return;
    }
    frx::shatter_into_pieces(ctx, sys, /* start piece */ 0, interiorMatId, shatterImpactProfile, props->frxDestr->shatterProfile,
      seed);
  ...
  void onDoneSync()
  {
    ...
    fill_destr_obj_params(destr, props->destrObj);
  ...
  const Point3 &impact_impulse)
{
  ...
  const rendinstdestr::RiDestrProps *props = rendinstdestr::get_ri_destr_props(buffer.desc);
  const rendinstdestr::FrxDestrProps *frxDestr = props ? props->frxDestr : nullptr;
  if (!frxDestr || !(frxDestr->flags & rendinstdestr::FrxDestrFlag::Shatter))
    return false;
  ...
  struct RiExtraCollisionCB final : public rendinst::RendInstCollisionCB
  {
    ...
    virtual void addCollisionCheck(const rendinst::CollisionInfo &coll_info) override
    {
      ...
      if (coll_info.desc != initiatorDesc && coll_info.isDestr && !coll_info.isImmortal && coll_info.destructibleByParent &&
          strcmp(rendinst::getRIGenDestrPropsTag(initiatorDesc), rendinst::getRIGenDestrPropsDestroyedByTag(coll_info.desc)) == 0)
  ...
  const DynamicPhysObjectData *po_data, const TMatrix &tm, rendinst::DestrOptionFlags flags)
{
  ...
  // for compatibility reasons riGen keeps the built-in lifetime, presets apply to riExtra only
  const rendinstdestr::RiDestrProps *props = desc.isRiExtra() ? rendinstdestr::get_ri_destr_props(desc) : nullptr;
  fill_destr_obj_params(params, props ? props->destrObj : nullptr);
  ...
  ApexDmgInfo *apex_dmg_info, rendinstdestr::on_destr_callback on_destr_cb, rendinst::DestrOptionFlags flags)
{
  ...
  // visual consumers span the drawn crown whatever box the destroying path asked for
  const BBox3 fullBBox = flags & rendinst::DestrOptionFlag::UseFullBbox ? bbox : rendinst::getRIGenFullBBox(desc);
  ...
  // dynamic RI destruction is handled fully inside callbacks, so exit for dynamic RIs
  {
    ...
    {
      rendinstdestr::call_on_rendinst_destroyed_cb(desc.getRiExtraHandle(), mainTm, bbox, fullBBox);
...
// captured before returnRIGenExternalControl, fired only when it succeeds: one home for
// the payload both producer sites hand the restored callback
struct RestoredRiPayload
{
  rendinst::RendInstDesc desc;
  TMatrix tm;
};
static RestoredRiPayload restored_ri_payload(const rendinst::DestroyedRi &ri)
{
  RestoredRiPayload p;
  p.desc = ri.savedData.desc;
  v_mat_43cu_from_mat44(p.tm.array, ri.savedData.tm);
  return p;
}
...
void rendinstdestr::update(float dt, float current_time, const Frustum *frustum)
{
...

  {
    ...
    for (int i = 0; i < riPhys.size(); ++i)
    {
      ...
      bool physObject = phys.isPhysObject();
      ...
      if (phys.ttl < 0.f || !isRiExtraValid)
      {
        if (!physObject)
        {
          const RestoredRiPayload restored = restored_ri_payload(*phys.ri);
          if (rendinst::returnRIGenExternalControl(phys.desc, phys.ri))
          {
            if (on_ri_restored_cb)
              on_ri_restored_cb(restored.desc, restored.tm);
    ...
    if (!updateList.empty())
    {
      ...
      for (int i = 0; i < updateList.size(); i++)
      {
        ...
        // Avoid setting zero prev matrix when showing a hidden tree again
        const bool wasHidden = prevTm.getcol(0).lengthSq() <= 0.f;
        if (wasHidden)
          prevTm = l.tm;
...
int rendinstdestr::test_dynobj_to_ri_phys_collision(const CollisionObject &coA, const TMatrix &tmA, float max_rad)
{
  ...
  for (int i = 0; i < riPhys.size(); ++i)
  {
    ...
    if (lengthSq(physTm.getcol(3) - tmA.getcol(3)) > sqr(max_ri_phys_rad + max_rad))
      continue;
...
void rendinstdestr::gather_ri_phys_for_dynobj_test(Tab<Point3> &out_pos)
{
  for (const RendInstPhys &phys : riPhys)
  {
    if (phys.isPhysObject())
      continue;
    TMatrix physTm = TMatrix::IDENT;
    dacoll::get_coll_obj_tm(phys.riColObj, physTm);
    out_pos.push_back(physTm.getcol(3));
  }
}

bool rendinstdestr::is_dynobj_near_ri_phys(dag::ConstSpan<Point3> ri_phys_pos, const Point3 &pos, float max_rad)
{
  const float maxDistSq = sqr(max_ri_phys_rad + max_rad);
  for (const Point3 &riPos : ri_phys_pos)
    if (lengthSq(riPos - pos) <= maxDistSq)
      return true;
  return false;
}
  ...
  const rendinst::CollisionInfo *coll_info, bool create_destr, bool from_damage)
{
  ...
  if (phys.isPhysObject())
  {
    ...
    const char *destrFxTemplate = coll_info ? rendinst::getRIGenDestrPropsFxTemplate(coll_info->desc) : "";
    if (!constrained_phys && coll_info && (coll_info->destrFxId >= 0 || *destrFxTemplate) && ri_effect_cb)
      ri_effect_cb(coll_info->destrFxId, TMatrix::IDENT, tm, coll_info->desc.pool, false, nullptr, destrFxTemplate);
  ...
  const rendinst::CollisionInfo *coll_info, bool create_destr, bool from_damage)
{
  ...
  for (int iTreeSlice = 0; iTreeSlice < ris.size(); iTreeSlice++)
  {
    ...
    if (phys.isPhysObject())
    {
      ...
      const char *destrFxTemplate = coll_info ? rendinst::getRIGenDestrPropsFxTemplate(coll_info->desc) : "";
      if (!constrained_phys && coll_info && (coll_info->destrFxId >= 0 || *destrFxTemplate) && ri_effect_cb)
        ri_effect_cb(coll_info->destrFxId, TMatrix::IDENT, tmNoScale, coll_info->desc.pool, false, nullptr, destrFxTemplate);
    }
    else
    {
      float hh = height * 0.5f;
