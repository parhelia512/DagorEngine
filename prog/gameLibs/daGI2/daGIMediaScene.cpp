...
void DaGIMediaScene::setClipVars(int clip_no, float voxel_size) const
{
...
#undef VAR
  ...
  // TEXFMT_R11G11B10F
  dagi_media_scene =
    dag::create_voltex(clipW, clipW, fullAtlasResD, TEXCF_UNORDERED | TEXFMT_A16B16G16R16F, 1, "dagi_media_scene", RESTAG_DAGI2);
  ...
  ShaderGlobal::set_float4(dagi_media_scene_atlas_decodeVarId, clipInAtlasPart, clipWithBorderInAtlasPart, 1.f / fullAtlasResD, 0);
  afterReset();
}
...
bool DaGIMediaScene::updateClip(uint32_t clip_no, const Point3 &world_pos, const prepare_initial_media_cb &prepare_media_cb)
{
  ...
  float voxelSize = get_voxel_size(clip_no);
  auto &clip = clipmap[clip_no];
  auto changed = clip.updatePos(world_pos, clipW, clipD, voxelSize);
  int changedCnt = changed.size();
  if (changedCnt == 0)
    return false;

  setClipVars(clip_no, voxelSize);
  ...
  for (int ui = 0; ui < changedCnt; ++ui)
  {
    ...
    ShaderGlobal::set_int4(dagi_media_scene_update_sz_coordVarId, updateSize.x, updateSize.y, updateSize.z,
      bitwise_cast<uint32_t>(voxelSize));
...
void DaGIMediaScene::resetHistoryAge()
{
  for (int i = 0, ie = clipmap.size(); i < ie; ++i)
  {
    ...
    setClipVars(i, get_voxel_size(i));
  }
}
...
void DaGIMediaScene::invalidateBox(const BBox3 &box)
{
  for (int clipNo = 0; clipNo < clipmap.size(); clipNo++)
    clipmap[clipNo].invalidateBox(box, clipW, clipD, get_voxel_size(clipNo));
}

bool DaGIMediaScene::isValid() const
{
