...
#include <render/obstacleStruct.hlsli>
...
ECS_TAG(render, dev)
ECS_NO_ORDER
static void gpu_deform_objects_manager_draw_debug_geometry_es(const ecs::UpdateStageInfoRenderDebug &, const TMatrix &transform,
  const Point3 &deform_bbox__bmin, const Point3 &deform_bbox__bmax)
{
  if (!show_obstacles)
    return;
