...
void AnimV20::FootLockerIKCtrl::clearAllocatedMemory(AnimGraphStateHolder &st)
{
  LegData *legsData = static_cast<LegData *>(st.getInlinePtr(legsDataVarId));
  for (int legNo = 0, numLegs = legsNodeNames.size(); legNo < numLegs; legNo++)
  {
    ...
    if (!leg.inited)
      continue;
    if (leg.toeHeightmapQueryId >= 0)
      heightmap_query::cancel_query(leg.toeHeightmapQueryId);
    if (leg.ankleHeightmapQueryId >= 0)
      heightmap_query::cancel_query(leg.ankleHeightmapQueryId);
    leg.toeHeightmapQueryId = leg.ankleHeightmapQueryId = -1;
  }
}
...
static void submit_heightmap_queries(AnimV20::FootLockerIKCtrl::LegData &leg, const AnimCharV20::TraceRayInfo &toeTrace,
  const AnimCharV20::TraceRayInfo &ankleTrace)
{
  G_ASSERT(leg.isLocked);
  if (leg.toeHeightmapQueryId >= 0)
    heightmap_query::cancel_query(leg.toeHeightmapQueryId);
  if (leg.ankleHeightmapQueryId >= 0)
    heightmap_query::cancel_query(leg.ankleHeightmapQueryId);
  leg.toeDisplacement = leg.ankleDisplacement = 0.f;
  // Use trace hit position for query, this way we get distance to the ground in `hitDistNoOffset`
  // and if it will be close to zero, we can apply heightmap displacement. Otherwise we are not on
  // the ground and should preserve old locked position.
  // Toe trace is guaranteed to hit because leg is locked now. Ankle trace could miss, but we do not care,
  // because most likely query is far from the ground too.
  leg.toeHeightmapQueryId = heightmap_query::query(toeTrace.pos - Point3(0, toeTrace.t, 0), Point3(0, -1, 0));
  leg.ankleHeightmapQueryId = heightmap_query::query(ankleTrace.pos - Point3(0, ankleTrace.t, 0), Point3(0, -1, 0));
}

static void get_heightmap_query_result(int &query_id, float &displacement)
{
  if (query_id < 0)
    return;
  HeightmapQueryResultWrapper hmapQueryRes;
  GpuReadbackResultState resState = heightmap_query::get_query_result(query_id, hmapQueryRes);
  G_ASSERTF(resState != GpuReadbackResultState::ID_NOT_FOUND && resState != GpuReadbackResultState::SYSTEM_NOT_INITIALIZED,
    "resState=%d, query_id=%d", (int)resState, query_id);
  if (is_gpu_readback_query_successful(resState))
  {
    constexpr float CLOSE_TO_HEIGHTMAP_THRESHOLD = 0.01f;
    displacement = fabsf(hmapQueryRes.hitDistNoOffset) < CLOSE_TO_HEIGHTMAP_THRESHOLD ? hmapQueryRes.hitDistWithOffsetDeform : 0.f;
    query_id = -1;
  }
  else if (is_gpu_readback_query_failed(resState))
  {
    displacement = 0.f;
    query_id = -1;
  }
...
void AnimV20::FootLockerIKCtrl::process(AnimGraphStateHolder &st, real wt, GeomNodeTree &tree, AnimPostBlendCtrl::Context &ctx)
{
  ...
  LegData *legsData = static_cast<LegData *>(st.getInlinePtr(legsDataVarId));
  if (distanceFromCameraVarId >= 0)
  {
    float cameraDistance = st.getParam(distanceFromCameraVarId);
    wt *= cvt(cameraDistance, disableDistanceRange.x, disableDistanceRange.y, 1.f, 0.f);
  }
  if (wt < 1e-3f)
  {
  ...
  // animState copy zeroes legsData (PT_InlinePtrCTZ) without calling init(), so re-init here
  for (int i = 0; i < numLegs; ++i)
    if (!legsData[i].inited)
    {
      init(st, tree);
      break;
    }
  ...
  dag::RelocatableFixedVector<float, 2, true /*OF*/, framemem_allocator> traceDistances;
  traceDistances.resize(numLegs);
  ...
  for (int legNo = 0; legNo < numLegs; legNo++)
  {
    ...
    if (!leg.isLocked)
    {
      ...
      traceDistances[legNo] = traceDist;
  ...
  for (int legNo = 0; legNo < numLegs; legNo++)
  {
    ...
    float maxTraceDist = traceDistances[legNo];
    ...
    if (!leg.isLocked)
    {
      ...
      if (!leg.isProceduralStepActive)
      {
        ...
        // than lock/unlock cases. But in case of negative `toeDisplacement` (visual ground lower than collision),
        // we should take it into account and don't raise the foot too fast right after unlock
        if (footVerticalMove > max(0.f, -leg.toeDisplacement))
    ...
    if (!leg.isLocked && needLock && lockAllowed)
    {
      ...
      if (useHeightmapQueries)
        submit_heightmap_queries(leg, traces[toeTraceIdx], traces[ankleTraceIdx]);
    }
    else if (leg.isLocked)
    {
      vec3f lockedPosRel = v_sub(v_ldu(&leg.lockedPosition.x), worldOffset);
    ...
    if (hipMoveDownVarId >= 0 && leg.isLocked)
    {
      ...
      vec3f targetToHip = v_sub(hip.col3, v_sub(ankleTargetPos, v_ldu(&leg.posOffset.x)));
  ...
  if (hipMoveDownVarId >= 0)
  {
    ...
    // When legs are not locked, hip returns to 0. Move it slower than usual to decrease
    // hip amplitude when we locked legs low, unlocked and then locked low again.
    constexpr float HIP_VISCOSITY_MULT_FOR_UNLOCKED_LEGS = 3.0f;
    bool anyLegLocked = false;
    for (int legNo = 0; legNo < numLegs; legNo++)
      anyLegLocked |= legsData[legNo].isLocked;
    float hipViscosity = hipMoveViscosity * (anyLegLocked ? 1.0f : HIP_VISCOSITY_MULT_FOR_UNLOCKED_LEGS);
    hipMoveDown = approach(hipMoveDown, hipTargetMove, dt, hipViscosity);
...
void AnimV20::FootLockerIKCtrl::createNode(AnimationGraph &graph, const DataBlock &blk)
{
  ...
  node->legsDataVarId =
    graph.addInlinePtrParamId(String(0, "$%s", name), sizeof(LegData) * numLegs, AnimGraphStateHolder::PT_InlinePtrCTZ);
  ...
  node->disableDistanceRange = blk.getPoint2("disableAtDistanceFromCamera", Point2::ZERO);
  const char *distanceFromCameraParamName = blk.getStr("distanceFromCameraParamName", nullptr);
  if (distanceFromCameraParamName != nullptr && node->disableDistanceRange != Point2::ZERO)
    node->distanceFromCameraVarId = graph.addParamId(distanceFromCameraParamName, AnimGraphStateHolder::PT_ScalarParam);
  node->useHeightmapQueries = blk.getBool("useHeightmapQueries", false);
  graph.registerBlendNode(node, name);
}

using AnimV20::FootLockerIKCtrl;
AUTO_REGISTER_ANIM_BLEND_NODE(FootLockerIKCtrl, "footLockerIK");
