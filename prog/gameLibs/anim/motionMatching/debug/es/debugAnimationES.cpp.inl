...
#include <animation/featureNormalization.h>
#include <animation/motionMatchingMetrics.h>
#include <animation/animationSampling.h>
#include <util/dag_console.h>

template <class T>
inline void imgui_prepare_skeleton_visualization_ecs_query(ecs::EntityManager &manager, ecs::EntityId, T);

static void get_current_mm_pose(GeomNodeTree &mm_animated_tree, eastl::bitvector<> &nodemask,
  const MotionMatchingController &controller, const AnimV20::AnimcharBaseComponent &animchar)
{
...
static void get_selected_anim_pose(GeomNodeTree &mm_animated_tree, eastl::bitvector<> &nodemask, int clip_idx, int frame_idx,
  const AnimV20::AnimcharBaseComponent &animchar, const MotionMatchingController &controller)
{
...
ECS_TAG(render, dev)
ECS_NO_ORDER
static void debug_motion_matching_skeleton_es(const ecs::UpdateStageInfoAct &, ecs::EntityManager &manager,
  ecs::EntityId mm_imguiAnimcharEid, bool mm_visualization_show_skeleton, bool mm_visualization_show_skeleton_original,
  int mm_visualization__selectedClipIdx, int mm_visualization__selectedFrameIdx, ecs::Point3List &mm_visualization__skeletonNodeWpos,
  ecs::IntList &mm_visualization__skeletonNodeFilter)
{
  ...
  imgui_prepare_skeleton_visualization_ecs_query(manager, mm_imguiAnimcharEid,
...
template <class T>
inline void enable_mm_on_player_ecs_query(ecs::EntityManager &manager, T);
template <class T>
inline void disable_mm_on_player_ecs_query(ecs::EntityManager &manager, T);
...
static bool enable_motion_matching_cheat(const char *argv[], int argc)
{
  ...
  CONSOLE_CHECK_NAME("motion_matching", "toggle_on_player", 1, 4)
  {
    ...
    disable_mm_on_player_ecs_query(*g_entity_mgr, [&](ECS_REQUIRE(ecs::EntityId watchedByPlr) ECS_REQUIRE(
                                                    MotionMatchingController motion_matching__controller) ecs::EntityId eid) {
      mmWasEnabled = true;
    ...
    enable_mm_on_player_ecs_query(*g_entity_mgr, [&](ECS_REQUIRE(ecs::EntityId watchedByPlr) ecs::EntityId eid) {
...
template <class T>
inline void process_current_character_ecs_query(ecs::EntityManager &manager, ecs::EntityId, T);
template <class T>
inline void get_current_imgui_character_ecs_query(ecs::EntityManager &manager, T);
template <class T>
inline void update_clip_for_debug_trajectory_ecs_query(ecs::EntityManager &manager, T);
...
static void motion_matching_weights()
{
  ...
  get_current_imgui_character_ecs_query(*g_entity_mgr,
    [&selectedEntity, &prevTransition](ecs::EntityId mm_imguiAnimcharEid, int mm_visualization__prevTransitionClipIdx,
      int mm_visualization__prevTransitionFrameIdx) {
  ...
  process_current_character_ecs_query(*g_entity_mgr, selectedEntity,
    [prevTransition](const AnimV20::AnimcharBaseComponent &animchar, MotionMatchingController &motion_matching__controller,
      ...
      MotionMatchingSearchFeatures searchFeatures;
      prepare_features_for_mm_search(dataBase, motion_matching__goalFeature, searchFeatures);
      ...
      if (motion_matching__controller.hasActiveAnimation())
      {
        curClip = motion_matching__controller.getCurrentClip();
        curFrame = motion_matching__controller.getCurrentFrame();
        int closestFrame = motion_matching__controller.getClosestFrame();
        curMetric = prevTransition.x < 0
                      ? calculate_trajectory_features_cost(dataBase, curClip, closestFrame, currentWeights, searchFeatures)
                      : calculate_features_cost(dataBase, curClip, curFrame, currentWeights, searchFeatures);
      }


      MatchingResult currentState = {curClip, curFrame, FLT_MAX};
      MatchingResult result =
        motion_matching(dataBase, currentState, motion_matching__controller.currentTags, currentWeights, searchFeatures);
      ...
      ImGui::Text("        [clip, frame]: metric\n"
        ...
        curClip, curFrame, curMetric, result.clip, result.frame, result.metrica, motion_matching__controller.motionMatchingWeight);
      ...
      static bool showTransitionCost = true;
      static bool showNodePosition = false;
      static bool showNodeVelocity = false;
      static bool showLegsLockState = true;
      ImGui::SameLine();
      if (ImGui::Button("Tooltip data"))
        ImGui::OpenPopup("TooltipData");
      if (ImGui::BeginPopup("TooltipData"))
      {
        ImGui::PushItemFlag(ImGuiItemFlags_AutoClosePopups, false);
        ImGui::MenuItem("Transition cost", "", &showTransitionCost);
        ImGui::MenuItem("Node position", "", &showNodePosition);
        ImGui::MenuItem("Node velocity", "", &showNodeVelocity);
        ImGui::MenuItem("Legs lock state", "", &showLegsLockState);
        ImGui::PopItemFlag();
        ImGui::EndPopup();
      }
      ...
      for (int clipIdx = 0, clipCount = dataBase.clips.size(); clipIdx < clipCount; clipIdx++)
      {
        const AnimationClip &clip = dataBase.clips[clipIdx];
      ...
      int timeParamId = dataBase.clips[curClip].animTreeTimer;
      if (timeParamId >= 0)
      {
        float time = animchar.getAnimState()->getParam(timeParamId);
        ImGui::Text("%s (%f)", animchar.getAnimGraph()->getParamName(timeParamId), time);
        int fakeFrame = fmodf(time / dataBase.clips[curClip].animTreeTimerCycle, 1.0f) * dataBase.clips[curClip].tickDuration;
      ...
      for (int i = 0, clipCount = dataBase.clips.size(); i < clipCount; ++i)
      {
        int clipIdx = sortRecent ? recentClips[i] : i;
        const AnimationClip &nextClip = dataBase.clips[clipIdx];
        ...
        for (int frame = 0, frameOffset = 0; frame <= nextClip.tickDuration; frame++, frameOffset++)
        {
          if (frameOffset >= framesInOneLine)
          {
            frameOffset = 0;
            clipOffset++;
            ImGui::Text("--- (%d-%d)", frame, min(frame + framesInOneLine, nextClip.tickDuration));
          }
          float result, trajectory, pose;
          calculate_features_cost_detailed(dataBase, clipIdx, frame, currentWeights, searchFeatures, result, trajectory, pose);
          ...
          {
            tooltipMessage.printf(0, "frame: %d\n", frame);
            if (showTransitionCost)
            {
              tooltipMessage.aprintf(0,
                "cost: %f\n"
                "- trajectory: %f\n",
                result, trajectory);
              FrameFeatures goalFeaturesCopy = motion_matching__goalFeature;
              int featuresOffset = nextClip.featuresNormalizationGroup * dataBase.featuresSize;
              normalize_feature(make_span(goalFeaturesCopy.data), goalFeaturesCopy.data.data(),
                dataBase.featuresAvg.data() + featuresOffset, dataBase.featuresStd.data() + featuresOffset);
            ...
            if (showNodePosition || showNodeVelocity)
            {
              for (int i = 0, ie = motion_matching__goalFeature.nodeCount; i < ie; ++i)
              {
                tooltipMessage.aprintf(0, "%s\n", dataBase.nodesName[i]);
                if (showNodePosition)
                  tooltipMessage.aprintf(0, "  position: %@\n", dataBase.getNodePositionFeature(clipIdx, frame, i));
                if (showNodeVelocity)
                  tooltipMessage.aprintf(0, "  velocity: %@\n", dataBase.getNodeVelocityFeature(clipIdx, frame, i));
              }
            }
            if (showLegsLockState && frame < nextClip.tickDuration)
            {
              tooltipMessage.aprintf(0, "legs lock state: \n");
              for (int i = 0, numLegs = dataBase.footLockerNodes.size(); i < numLegs; i++)
                tooltipMessage.aprintf(0, " - leg %@: %@\n", i, dataBase.clips[clipIdx].footLockerStates.needLock(i, frame, numLegs));
            }
            update_clip_for_debug_trajectory_ecs_query(*g_entity_mgr,
              [clipIdx, frame](int &mm_visualization__selectedClipIdx, int &mm_visualization__selectedFrameIdx) {
                mm_visualization__selectedClipIdx = clipIdx;
                mm_visualization__selectedFrameIdx = frame;
              });
          }
          if (curClip == clipIdx && curFrame == frame)
          {
            curClipOffset = clipOffset;
            curFrameOffset = frameOffset;
          }
          if (clipIdx == prevTransition.x && frame == prevTransition.y)
          {
