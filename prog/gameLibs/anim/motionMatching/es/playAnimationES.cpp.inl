...
#include <animation/motionMatchingTags.h>
#include <animation/motionMatchingController.h>
#include <animation/motionMatchingAnimgraphNodes.h>
...
static void mm_update_goal_features_es(const ParallelUpdateFrameDelayed &act, FrameFeatures &motion_matching__goalFeature,
  const ecs::IntList &motion_matching__goalNodesIdx, bool motion_matching__enabled, const AnimV20::AnimcharBaseComponent &animchar,
  const MotionMatchingController &motion_matching__controller, const ecs::Point3List &mm_trajectory__featurePositions,
...
ECS_TAG(render)
ECS_AFTER(after_animchar_update_sync)
static void update_tag_changes_es(const ParallelUpdateFrameDelayed &, MotionMatchingController &motion_matching__controller,
  AnimV20::AnimcharBaseComponent &animchar, int &motion_matching__presetIdx, float &motion_matching__presetBlendTimeLeft,
  float &mm_trajectory__linearVelocityViscosity, float &mm_trajectory__angularVelocityViscosity, bool &motion_matching__enabled)
{
  if (!motion_matching__controller.dataBase)
    return;
  ...
  if (currentTags.wasChanged || motion_matching__presetIdx < 0)
  {
    ...
    motion_matching__presetIdx = dataBase.getPresetIdxFromTags(currentTags);
    ...
    motion_matching__presetBlendTimeLeft =
      max(max(preset.presetBlendTime, prevPreset.presetBlendTime), motion_matching__presetBlendTimeLeft);
...
void motion_matching_ecs_query(ecs::EntityManager &manager, T ECS_CAN_PARALLEL_FOR(c, 1));
...
public:
  ...
  void doJob() override
  {
    ...
    ecs::EntityManager::ScopeSetMtConstrained constrainedMTMode(*g_entity_mgr);
    motion_matching_ecs_query(*g_entity_mgr,
      [this, &processedMotionMatchingCount](MotionMatchingController &motion_matching__controller,
        ...
        if (motion_matching__updateProgress >= motion_matching__distanceFactor || animFinished)
        {
          ...
          {
            int curClip = motion_matching__controller.getCurrentClip();
            int curFrame = motion_matching__controller.getCurrentFrame();
            bool suitableTags = motion_matching__controller.isAnimationTagsMatch();
            float currentMetric =
              suitableTags ? calculate_trajectory_features_cost(dataBase, curClip, curFrame, currentWeights, searchFeatures) : FLT_MAX;
            currentState.clip = curClip;
            currentState.frame = curFrame;
          ...
          if (!DISABLE_MOTION_MATCHING && performSearch)
          {
            ...
            MatchingResult bestIndex2 = motion_matching_brute_force(dataBase, currentState, motion_matching__controller.currentTags,
              currentWeights, searchFeatures);

            G_ASSERT(bestIndex.clip == bestIndex2.clip && bestIndex.frame == bestIndex2.frame);
#endif
...
template <typename T>
void motion_matching_per_frame_limit_ecs_query(ecs::EntityManager &manager, T);

// Uses results of `update_tag_changes_es` and `mm_update_goal_features_es` from previous frame
ECS_TAG(render)
static __forceinline void motion_matching_job_es(const MotionMatchingStartSearchJob &act, ecs::EntityManager &manager)
{
  int perFrameLimit = 0;
  motion_matching_per_frame_limit_ecs_query(manager, [&perFrameLimit](int main_database__perFrameLimit) {
