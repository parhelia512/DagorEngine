...
bool avi_start_write(const DataBlock &stg)
{
  ...
  if (aviWriter->isInitializedOk())
  {
    ...
    old_acts_per_frame = dagor_get_fixed_act_per_frame();
    ...
    dagor_set_fixed_act_per_frame(stg.getInt("acts_per_frame", 2));
    return true;
  }
  delete aviWriter;
  aviWriter = NULL;
...
void avi_stop_write()
{
  ...
  dagor_set_fixed_act_per_frame(old_acts_per_frame);
}
#else
...
bool AviWriter::initialize(const DataBlock &blk)
{
  ...
  {
    resolvedRenderTargetTexture[i] =
      d3d::create_tex(NULL, screenWidth, screenHeight, TEXFMT_A8R8G8B8 | TEXCF_RTARGET, 1, "aviwriter", RESTAG_AVIWRITER);
