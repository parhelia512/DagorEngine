...
static constexpr char suiteTags[] = "[wsJsonRpcPersistentConnection][sendReceive]";
...
TEST_CASE("WsJsonRpcPersistentConnection: Receive unexpected data and discard it safely", suiteTags)
{
  ...
  SECTION("Skip setting incoming request callback") { connection.initialize(default_ws_config_generator, nullptr, nullptr); }

  {
    INFO("Wait until connected");
    connection.poll();
    ws.timer->addTimeMs(1000);
    REQUIRE(connection.isCallable() == false);
    connection.poll();
...
TEST_CASE("WsJsonRpcPersistentConnection: receive server-initiated JSON RPC messages (reverse direction)", suiteTags)
{
  WebsocketLibraryInterceptor ws;
  const auto wsMock = ws.addFutureConnection()->shouldSucceedConnectingAtMs(1000, 999'999);
  ...
  SECTION("Set incoming request callback")
  {
    connection.initialize(default_ws_config_generator, incomingRequests.createCallback(), nullptr);
  }

  SECTION("Skip setting incoming request callback")
  {
    haveIncomingRequestCallback = false;
    connection.initialize(default_ws_config_generator, nullptr, nullptr);
  }
...
TEST_CASE("WsJsonRpcPersistentConnection: execute RPC call in incorrect moment", suiteTags)
{
  WebsocketLibraryInterceptor ws;
  ...
  SECTION("Initiate connection close and execute RPC call after initiateClose")
  {
    ...
    REQUIRE(connection.isCallable() == false);
    REQUIRE(connection.isCloseCompleted() == false);
  ...
  {
    INFO("Wait until underlaying connection is closed (and reconnect will start if initiateClose was not called)");
    ws.timer->addTimeMs(1000);
    connection.poll();
    REQUIRE(connection.isCallable() == false);
  }
...
TEST_CASE("WsJsonRpcPersistentConnection: execute RPC call", suiteTags)
{
  WebsocketLibraryInterceptor ws;
  const auto wsMock = ws.addFutureConnection()->shouldSucceedConnectingAtMs(1000, 999'999);
...
TEST_CASE("WsJsonRpcPersistentConnection: execute RPC call with sudden disconnect before reply", suiteTags)
{
  WebsocketLibraryInterceptor ws;
  const auto wsMock = ws.addFutureConnection()->shouldSucceedConnectingAtMs(1000, 2000);
  const auto wsMock2 = ws.addFutureConnection()->shouldSucceedConnectingAtMs(5000, 999'999);
...
TEST_CASE("WsJsonRpcPersistentConnection: execute RPC call with initiated stop before reply", suiteTags)
{
  WebsocketLibraryInterceptor ws;
  const auto wsMock = ws.addFutureConnection()->shouldSucceedConnectingAtMs(1000, 2000);
