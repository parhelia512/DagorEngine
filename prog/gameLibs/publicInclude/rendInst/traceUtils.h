...
struct TraceMeshFaces
{
  ...
  public:
    ...
    __forceinline bool foreachValidUntil(rendinst::GatherRiTypeFlags ri_types, callback_t callback) const
    {
      for (const CacheEntry &entry : cachedData)
      {
        ...
        if ((entry.type & ri_types) && rendinst::isRiGenInWorld(entry.desc))
          if (callback(entry.desc, isRiExtra))
            return true;
      }
      return false;
    }
    ...
    {
      foreachValidUntil(ri_types, [&](const rendinst::RendInstDesc &desc, bool is_ri_extra) {
        callback(desc, is_ri_extra);
        return false;
      });
    }
  ...
  struct MaterialMapCache
  {
    ...
    int getMatAt(const Point2 &pos) const
    {
      ...
      constexpr uint8_t invalid_mat_id = 0xFF;
