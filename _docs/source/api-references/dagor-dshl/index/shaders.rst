...
Postfixes ``@smp2d, @smp3d, @smpArray`` ensure that a ``SamplerState`` object gets defined with texture/textures,
...
Note that you can use ``<texture_name>_samplerstate``, generated by the shader compiler, in ``hlsl{...}`` blocks
(e.g. ``hlsl_texture_samplerstate`` from the example).

Postfix ``@tex`` defines a texture of a specific type and must be followed by ``hlsl{...}`` block
...
.. code-block:: text
    ...
    block(frame|scene|object) name_of_block
    {
